...
**This plugin registers no rule, and the omission is a decision.** `applies` keys on `{{` plus a GB block rather than on tag names, so every tag we ship is already in scope and a rule of ours would be a second rule over identical content. **The trap worth naming: because the protocol is a `class_exists` check and not an action, nothing fires and nothing fails if a future maintainer assumes we participate.** There is no hook to grep for, no missing callback to notice. What stands in for it is `verify.php`'s P1 pair — the positive plus a control asserting that one of our tags OUTSIDE a GB block is NOT scanned — which is what turns "we are covered by GB's predicate" from an assumption into something that breaks loudly if GB ever narrows the predicate to a name list.
