...
- **`rows` is not offered on a base tag, and since 1.21.0 it still RENDERS.** `{{text}}`, `{{content}}`, `{{email}}` and `{{phone}}` read a `meta_row` chain — one named sub-field per row — and every container that absorbs the text read (`{{join}}` slots, `try_` attempts) came with it. `{{title}}`, `{{permalink}}`, `{{image}}` and the two `{{datetime_*}}` families REFUSE a `rows` chain and render nothing — their stated fallback where the family has one, the editor preview label where it has that. **Until 1.21.0 they did not render empty, they printed the SURROUNDING PAGE**: the page's own title, address, image field or date, because a `rows` chain resolves no post and the post tail read the ambient entity. Neither family will gain a row arm for its analog spellings — a row is not an entity — and `{{image}}`/`{{datetime_*}}` gain keyed row reads with FW-74 tickets 05 and 06. **The row read takes each family's OWN cardinality**, not a rule of its own: `{{text}}`/`{{email}}`/`{{phone}}` fan across the rows with the tag's `limit` and `sep` applying, while `{{content}}` collapses to the first row, because it is `takes_first_usable` (ADR 0007) and registers neither key. An ANALOG refuses on a row (`use:title`, and `{{content}}`'s `use:content`/`use:excerpt`, render nothing): a row is not an entity, and the hop those spellings imply is already sayable — `src:rows,team_members;refs,lead_ref|use:title`. What `{{table}}` owns is ASSEMBLY — a row across many columns — not the read. The step is on no offer, so authoring one still needs a hand edit.
