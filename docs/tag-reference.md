...
**And it is Tag-Converter-ONLY, for the same reserved-key constraint as the fold, reached by a different route.** The completion touches only `as`, so an editor mount effect looks reachable where the fold's is not. It is not safe: `tagSpecificControls` receives only `{ state: extraTagParams, setState }`, and `size` was destructured into GB-private `imageSize` before that object existed, so **in the editor a legacy split tag is indistinguishable from a size-less one**. Writing `url,full` on mount would pin such a tag's render to full while the read seam had been resolving it at `medium` — a silent downgrade, on open rather than on edit. The converter avoids that by ORDER: its fold entry is registered first, so a tag carrying both is folded to `as:url,<authored size>` and never reaches the completion. The editor has nothing to order against, because the thing to order against is the key it cannot see.

One consequence to know when editing a legacy split tag: the composite has always written `as:url,full` when the author touches a control, and that has always dropped the authored size the same way. **Run the Migration Tool before opening such tags.** The editor route is not a substitute, and a tag in a block widget (out of the scanner's reach) keeps its bare `as:url` until an author edits it.
