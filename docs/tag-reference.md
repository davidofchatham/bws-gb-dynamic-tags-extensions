...
| `bws-term-hop` | CheckboxControl + ComboboxControl over public taxonomies (via `wp.data` `core`). Reads `pickLabel` / `pickHelp` from PHP option config in addition to `label` / `help` | `assets/js/term-hop-control.js` | `srcTermIn` option on base tags + per-slot in try_ tags. FW-129 removed its `term_*` home; whether the control itself survives is the census at the end of that sweep |
