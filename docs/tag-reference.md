...
- **`rows` is not offered on a base tag, and since 1.21.0 it still RENDERS.** `{{text}}`, `{{content}}`, `{{image}}`, `{{email}}` and `{{phone}}` read a `meta_row` chain — one named sub-field per row — and every container that absorbs the text read (`{{join}}` slots, `try_` attempts) came with it. `{{title}}`, `{{permalink}}` and the two `{{datetime_*}}` families REFUSE a `rows` chain and render nothing — their stated fallback where the family has one, the editor preview label where it has that. **Until 1.21.0 they did not render empty, they printed the SURROUNDING PAGE**: the page's own title, address, image field or date, because a `rows` chain resolves no post and the post tail read the ambient entity. The first two will never gain a row arm — their spellings are all analogs, and a row is not an entity — while `{{datetime_*}}` gains a keyed row read with FW-74 ticket 06. **The row read takes each family's OWN cardinality**, not a rule of its own: `{{text}}`/`{{email}}`/`{{phone}}` fan across the rows with the tag's `limit` and `sep` applying, while `{{content}}` and `{{image}}` collapse to the first row, because both are `takes_first_usable` (ADR 0007) and register neither key. An ANALOG refuses on a row (`use:title`, `{{content}}`'s `use:content`/`use:excerpt` and `{{image}}`'s `use:featured` all render nothing): a row is not an entity, and the hop those spellings imply is already sayable — `src:rows,team_members;refs,lead_ref|use:title`. **The image read is the one that needed the raw read seam**, because an image sub-field's stored shape follows its own field's return format — an array under ACF's default — and the string seam every other row arm reads through drops arrays; `bws_read_resolved_source_value()` is the half that does not. What `{{table}}` owns is ASSEMBLY — a row across many columns — not the read. The step is on no offer, so authoring one still needs a hand edit.
