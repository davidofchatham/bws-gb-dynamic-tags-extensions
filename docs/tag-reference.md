...
- **`rows` is offered on a base tag since 1.21.0, and it RENDERS.** `{{text}}`, `{{content}}`, `{{image}}`, `{{email}}`, `{{phone}}` and both `{{datetime_*}}` families read a `meta_row` chain — one named sub-field per row — and every container that absorbs the text read (`{{join}}` slots, `try_` attempts) came with it. `{{title}}` and `{{permalink}}` REFUSE a `rows` chain and render nothing — their stated fallback where the family has one, the editor preview label where it has that. **Until 1.21.0 they did not render empty, they printed the SURROUNDING PAGE**: the page's own title, address, image field or date, because a `rows` chain resolves no post and the post tail read the ambient entity. Neither will ever gain a row arm — their spellings are all analogs, and a row is not an entity. **The row read takes each family's OWN cardinality**, not a rule of its own: `{{text}}`/`{{email}}`/`{{phone}}`/`{{datetime_*}}` fan across the rows with the tag's `limit` and `sep` applying, while `{{content}}` and `{{image}}` collapse to the first row, because both are `takes_first_usable` (ADR 0007) and register neither key. An ANALOG refuses on a row (`use:title`, `{{content}}`'s `use:content`/`use:excerpt` and `{{image}}`'s `use:featured` all render nothing): a row is not an entity, and the hop those spellings imply is already sayable — `src:rows,team_members;refs,lead_ref|use:title`. **The image read is the one that needed the raw read seam**, because an image sub-field's stored shape follows its own field's return format — an array under ACF's default — and the string seam every other row arm reads through drops arrays; `bws_read_resolved_source_value()` is the half that does not. **The datetime read takes the VALUE half of that seam only**: each core keeps deriving its own custom-fields object id, a separate argument from the value read, and a row reaches no sub-field config — so a date sub-field on a wire row is parsed FORMAT-AGNOSTICALLY, and the field-object read that would close it is FW-3's. What `{{table}}` owns is ASSEMBLY — a row across many columns — not the read. **The step joined every chain-step offer in 1.21.0** — base tags, `{{join}}` fields and `try_` attempts alike, the three lists being asserted equal — once every keyed arm read a row; it was held back only while nothing consumed one, because a step no arm reads authors a chain that renders empty.
...
**Strip-default caveat.** The analog is reached by the tag's *stripped-default* `use` value, which is its **first enum value** — and that first value is **key-mode** for `text` and `image` (so their analog is NOT the empty-wire default). An empty `use` therefore resolves to key-mode for text/image (read a `key`), to `content` for `content` (which, under `src:site`, has no analog → empty). The site **logo** is the explicit `use:featured` value, *not* the implicit-mode `{{image src:site}}` (which is key-mode → empty without a key). Key-mode was first kept as the default for a reason that no longer holds: a `key` left behind when `use` moved to an analog could not be told from intended key-mode. The editor now deletes that `key` when `use` leaves key-mode ([`editor-controls.md` §The `use` read wrapper](editor-controls.md#the-use-read-wrapper)). `text` stays key-mode because it is primarily a meta-field read; flipping `image` to lead with `featured` is tracked as FW-143 ([`future-work.md`](future-work.md)).
...
| `src:site` on a **single-slot rooted modifier** (`term_*`, or an externally registered one) | **No** — the site datum is the *identical* read the unrooted base tag gives (`{{email src:site}}`); the term/view rooting is discarded | **No** — site is entity-blind, so it fills no term-/view-distinct slot; a rooting modifier exists to surface entity-distinct data | **rejected** — `site` is filtered from the modifier's `src` dropdown. *Likely future home for "specific resource + site fallback" is an ID source (a probable `src:<type>,<ID>`-style construct — **not final**; the author-serialized-entity-id flavor, see CONTEXT.md §Language "Source binding") as a try_ attempt (which keeps its site attempt via `try_allow_site_slot`), NOT a `try_term_` form — `term_` is a transitional N×M surface on a deprecation glide-path (base tags + context-awareness + a specific-entity source subsume it).* |
...
**Per-user REST gate (option reads), 1.19.2.** A second, independent gate sits beside the allowlist above and applies at the same three reads. Under a **REST request**, for a user who can `edit_posts` but may not author dynamic data, an option read resolves to the tag's configured fallback instead of the value — unless the key is one GB Pro would still disclose to that user, which in practice means the six WP defaults stay readable and registered ACF options-page fields do not. Front-end rendering is untouched, anonymous REST is untouched, and which keys are readable at all does not change. The two axes are independent: the allowlist asks *which keys*, this asks *which user*, and being stricter than GB Pro on the first buys nothing against the second. Mirrors GB Pro 2.7's own conditional, including its scope. See [`docs/adr/0008-site-option-per-user-rest-gate.md`](adr/0008-site-option-per-user-rest-gate.md).
...
**The source gate** applies at EVERY chain position, per step, before that step's limit, and what an author observes is this: an author previewing their own draft resolves it, a visitor does not resolve drafts or private posts, and trashed content resolves for nobody. An entity failing the gate cannot be a stepping stone — the chain is cut at that hop even toward public targets. The deciding rule (which kinds are tested, and against what) is owned at the gate itself, `bws_source_gate()`'s PHPDoc in `includes/helpers/traversal-pipeline.php`. **Caching note:** the visible test is viewer-relative, so a full-page cache serves whichever viewer primed the page; a cache that does not vary on login state will show every visitor the logged-out render, which is the safe direction, but a cache primed by a logged-in view can persist privileged output. This is guidance for debugging a cache, not a plugin setting.
...
- **Only where the slot's OWN chain fans.** A slot spelling `src(same)`, or an argless `src(refs)`, fans solely by CARRYING OVER an earlier slot's source. Giving it the chain default would make a migrated `{{join A:src(refs,office,limit[1])…|B:src(same)…}}` return every related post at B where the flat wire it replaced returned one. The predicate (`bws_fold_chain_fanning_steps()`) is shared with the migrator's stamp, so a slot can never take a default the migration declined to state.
- **A SELECTING container's slot carries the BOUND along with the source** (1.17.0, #61). `src(same)` names the same source and a limit is one of a source's parameters, so `bws_fold_slot_chain_options()` carries the quantity the earlier slot resolved. That is what let `try_`'s tag-level `limit` be retired without moving output. `{{join}}`/`{{table}}` are the deliberate contrast: they own `limit` PER SLOT (`{N}-limit`), so an absent one there is a slot saying "I state none" and must take the default — carrying it would move shipped `{{join}}` output.
...
| value brackets | `()` at level 1, `[]` at level 2 | **alternation by depth**, never a fixed char: `limit(3)` on a base tag's own `src:`, `limit[3]` inside a slot's `src(…)` |
...
**Step slugs are wire vocabulary, and the engine's step types follow them:** `refs` (relationship), `terms` (taxonomy), `rows` (repeater rows), plus `same` (carry-over) and the base `src` values (`current`, `site`, …). One map holds the correspondence (`BWS_FOLD_STEP_TYPES`), so the layers *can* diverge; the values are deliberately identical so a reader has no translation to hold.
...
**The slot count is explicit.** The `bws-slot-fold` repeater adds and removes slots, and removal compacts (closing the hole re-points `same` backreferences, so carried axes are materialized against the removed slot first). Nothing is inferred from how far configuration got, which is why the old combining reveal predicates are gone.
...
1. **Source chain** — `src(…)`, or absent. Slot 1 absent = the ambient entity; slot 2+ absent carries over the previous resolving slot, and `src(same)` says so explicitly. The `terms` step is offered as a chain step (`src(refs,rel;terms,category)`); it names **this** slot's entity and never carries forward.
...
|---|---|---|---|
| `use` enum + key picker | `try_text`, `try_content`, `try_image` | mode select, plus the field picker when the mode needs a key | carry-over (materialized as `use(same)`) |
| key picker alone | `try_email`, `try_phone` | field picker only — no mode axis exists | carry-over (an empty picker IS the carry-over) |
