...
- **Read seam:** cores parse `as` via `bws_parse_as_option()` (`includes/helpers/image-helpers.php`), which splits `mode,size` and falls back to a legacy separate `size:` then `full`, so pre-fold saved tags still render at their size.
- **Migration:** `bws_migrate_image_as_size_fold()` (a `transform_callback` migration, `includes/tags/deprecated-tags.php`) folds a legacy `size:` (bare or per-slot `N-size:`) into `as` value-conditionally — into `url,<size>` for url, dropped for a nullary mode (size was dead there). Runs on editor reopen (composite writes) or via the Tag Converter.
...
**Format-front is a deliberate departure from GB.** GB's own `post_date` (its one tag carrying both a format control and link) serializes `link` BEFORE `dateFormat` (format-LAST): `{{post_date id:100|link:author_archive|dateFormat:F j, Y}}`. We invert to format-FIRST for manual-edit copy-visibility. Everything else (`source → field → link`) mirrors GB's chain.
...
**Serialization order ≠ control order is intentional** — and GB itself does this: `post_date`'s modal renders Date Format ABOVE Link To yet serializes link before format. Our control order is `source → format → link → fallback` (format-early in the panel, matching GB's own `post_date`); our serialization order lifts `format` to front (`format → source → link → fallback`). The two are reconciled by a per-tag JS normalizer that rebuilds `extraTagParams` in the canonical serialization order inside `setState` (transform = lift `format` to front + keep each `N-` slot contiguous). Status, scope, and the build constraints (per-tag gating, Strategy-1 `N-`-prefix block detection, registration-standardization unwind) live in [`.claude/plans/combined-option-controls.md` §Phase 3+ + §Grill outcomes 2](../.claude/plans/combined-option-controls.md) / FW-52 — **not yet built**; reconcile this section with code on ship (adopting the "control order" / "serialization order" terms + linking gb-constraints for GB mechanics).
