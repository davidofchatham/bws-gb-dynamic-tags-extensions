...
| explicit `0` / `-1` | no STEP limit — unlimited is what chain wire already means — and at depth 0 the key stays as written. In a SELECTING container the key is retired anyway (#61): a slot recovered from LEGACY keys keeps an explicit `limit[0]` carrier, because that same mapper renders unmigrated flat wire; an already-folded slot needs none, since its chain already defaults to unlimited |
