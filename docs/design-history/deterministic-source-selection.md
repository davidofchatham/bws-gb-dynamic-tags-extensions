...
`(bool $ok, array $source)`. Grounded rather than guessed: the only known consumer is the sister plugin, whose predicate is

```php
is_post_visible( int $post_id, ?string $scope_id, string $post_type ): bool
```

It derives `$scope_id` from ambient request context itself and `$post_type` from the id, and already exposes its own visibility filter — so composition is filter-to-filter and **the minimum it needs from us is the post id.** A resolved source carries `kind` + `id`, which covers it with nothing to spare.
