...
| `bws_get_loop_row_context()` | Detect a GB query-loop item and its shape (a post, a term, a user or a repeater row — see the contract note below) |
...
**`bws_get_loop_row_context()` — the states a caller can meet (v1.19.0).** The loop item may now hold
...
`row_post_id` holds a post and only a post, so it is `false` while `in_loop` is `true` for every term
and user loop — **an ordinary state, not an edge case**, and code that reads `row_post_id` as "the
