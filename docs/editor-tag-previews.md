...
| A PINNING root with no argument (1.20.0, FW-39) — e.g. bare `src:term` or `src:post`, hand-wire only | `⚠ Term: nothing pinned` / `⚠ Post: nothing pinned` | `B nothing pinned` |
...
The internal tokens `current`, `site` and `ref` never flag — they resolve. `post` and `term` are internal tokens too, but BOTH are also pinning roots (1.20.0, FW-39): a *bare* `post`/`term` is that pin left unfilled, and the row above (`A PINNING root with no argument`) is what fires for it — not this silence.
