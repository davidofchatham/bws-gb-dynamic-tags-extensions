...
| A DECLARING root with no argument (1.20.0, FW-39) — e.g. bare `src:term` or `src:post`, hand-wire only | `⚠ Term: nothing selected` / `⚠ Post: nothing selected` | `B nothing selected` |
