...
| chain step `entries,<field>` | chain step `rows,<field>` | `{{table}}` (step 6); legal wire elsewhere | Renamed 2026-08-22 — **no migration entry, deliberately** | Introduced as new vocabulary in 1.17.0 spelled `entries`, renamed because the word was already carrying a second shipped sense (what a relationship field holds — `single-entry`, “all entries will be results”). Safe without a migration because the slug was on NO step offer: both authoring surfaces pass `['refs','terms']`, so no editor ever wrote it and only a hand edit could. Promotes a repeater to a traversal step; a `try_`/join slot holding one still renders nothing (no arm assembles a repeater row). |
