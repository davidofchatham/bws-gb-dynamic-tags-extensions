...
| Template | Supports | Options | `post_` | `related_post_` | `second_related_post_` | `post_term_related_post_` | external |
|---|---|---|---|---|---|---|---|
...
**`term_*` IS migrated, from 1.20.0 (FW-39).** It is the first in-repo family to use this API, and it registers its entries through the same generator an external owner calls. Two shapes the external worked example below does not cover, because its root takes no argument: an ARGLESS `{{term_*}}` tag converts to a bare base tag with no source at all, and one NAMING A TERM (`id:34`) converts to `{{text src:term,34}}` — the root's argument, not GB's `id` key, which is dropped. A taxonomy set beside a term id is dropped too; a term id is unique on its own. Three hand-typed shapes are SKIPPED whole rather than converted (`bws_modifier_skip_reason()` owns which and why). The argless rewrite is the one place this repo's migrations are not output-neutral, which was measured rather than assumed — build record in [`docs/design-history/term-family-migration-output-neutrality.md`](design-history/term-family-migration-output-neutrality.md). `fixture_*` exists only in the test blueprint.

**No EXTERNAL family is registered for this migration today.** The one integrating plugin deleted its modifier family AND its `bws_register_modifier_root_migrations()` companion in the same release (2026-09-08), so one of its tags still stored in content neither renders (nothing registers the name — GB emits the literal string) nor converts (no entry describes the rewrite). That is the owner's decision about the owner's names; recorded here because the rows below now read as a worked example rather than as a conversion an external plugin can still perform. Retiring the API these rows describe is `future-work.md` FW-129.
...
A row above states one entry's rewrite. An older prefix pointing at a newer one also reaches the base tag in a single run, since the converter re-reads the tag name after each rewrite; see [§9 in `plugin-integration.md`](plugin-integration.md#9-migrating-a-modifier-family-to-a-base-tag).
...
| `srcTermIn` (slug) | step `terms,<tax>` inside the `src` value | `srcTermIn` / `N-srcTermIn`, on any tag | Implemented (v1.17.0) | **Second rename for this key** (`srcTerm`+`tax` → `srcTermIn` → a chain step), and a shape change like the `refs` row above, so a migrator must handle both eras. A legacy `srcTermIn` is APPENDED to a chain that has no `terms` step of its own — it is a separate option KEY describing a step, so dropping it would lose a configured step, and appending it beside an existing one would double-hop. **A SITE root never takes the legacy term step:** the pair is hand-edit-only (`srcTermIn` registers `show_if src: not:site`) and every arm has always let the site read win, so appending there would flip a stored tag from "the site value" to empty. The `bws-term-hop` control that authored the flat key is **gone as of v1.21.0** (FW-67), along with the families that never took a chain source and so kept it alive (`term_` and `{{table}}`; an external prefix was the third until its owner deleted it, 2026-09-08). Nothing offers the flat key any more; the three processing allowances for stored wire — the compiler's appended `terms` step, the fold migration's conversion, and the slot-fold control's deletion of a stale control — are unaffected. |
...
| chain step `entries,<field>` | chain step `rows,<field>` | `{{table}}` (step 6); legal wire elsewhere | Renamed 2026-08-22 — **no migration entry, deliberately** | Introduced as new vocabulary in 1.17.0 spelled `entries`, renamed because the word was already carrying a second shipped sense (what a relationship field holds — `single-entry`, “all entries will be results”). Safe without a migration because **NOTHING SHIPPED CONSUMED A REPEATER ROW AT THE TIME OF THE RENAME** (2026-08-22; `{{text}}` and the slots absorbing its read consume one since 1.21.0, FW-74, which does not disturb the waiver — the wire that existed then rendered nothing under either spelling). The step promotes a repeater to a traversal step, and the only tag that turns rows into output is `{{table}}`, whose registration is gated behind `apply_filters( 'bws_dynamic_tags_register_table_tag', false )` — off on every install but the fixture testbed. Everywhere else the step resolves to a `meta_row` no arm assembles (FW-74), so a `try_`/join slot holding one renders nothing. Stored `entries` wire therefore produced no output before the rename and produces no output after it: the net delta is zero, which is the CHANGELOG's own test. **The weaker reason is not the one to lean on:** the slug was also on NO step offer at the time (every authoring surface passed `['refs','terms']` until 1.21.0, so only a hand edit could write it) — but ADR 0004 makes hand-edited wire a first-class authoring route, so "no control wrote it" would not have earned the waiver on its own. |
...
| `sep` (folded-slot spelling) | *(held)* | folded slot values | Under consideration | Deferred with the five datetime keys; see the plan's OPEN table. The tag-level `sep`/`valueSep` names are unaffected. Already half-built and worth knowing before the rest lands: a slot `sep(...)` **parses into the struct's `opts` and round-trips through emit today** — ADR 0003 is enforced in ONE place — the render seam, which drops it on the way to the renderer in both modes. That place is `bws_fold_slot_chain_options()` since 1.17.0 ([#104](https://github.com/davidofchatham/bws-gb-dynamic-tags-extensions/issues/104)); it was `bws_fold_slot_flat_options()` when this row was written, and the enforcement point moved with the deletion rather than going away. Shipping it is a seam change, not a wire change. |
...
|---|---|---|---|---|
| `omit_current_year` | `showCurrentYear` | date/datetime | Approved | Flip boolean: unset = omit current year (default smart behavior); presence = always show year. Fixes `default:true` serialization problem. Revised from `show_current_year` → camelCase to align with GB JS option convention. Shim: `$options['showCurrentYear'] ?? $options['show_current_year'] ?? ''`. Converter: if `omit_current_year` reads FALSY on the old tag (absent, `''` or `'0'` — the pre-1.6 renderer read `! empty()` and GB merged no defaults, so all three rendered the year), inject `showCurrentYear` (bare key) to preserve "always show year" behavior; if truthy, drop it. Two gates, both added in 1.17.0 ([#90](https://github.com/davidofchatham/bws-gb-dynamic-tags-extensions/issues/90)): injection needs ERA evidence, since absence alone cannot tell legacy wire with the box unchecked from a modern tag that never had the key (a pre-1.6 tag NAME is evidence; on the two `type:'option'` entries the evidence list is `datetime_era_options`, which excludes `fallback_text` because that key is renamed on every base tag); and it is skipped on a `time`-only tag, which renders no year. The shape is read from the ENTRY's `fixed_options` before the wire's own `as`, because `fixed_options` are assigned unconditionally at the end of the pipeline and therefore decide what the migrated tag actually is. |
...
| `smart_time` | *(eliminated)* | datetime | Approved — **fully implemented 1.19.1** | AM/PM consolidation becomes always-on (ungated). Midnight suppression becomes `showMidnight` option. `smart_time` eliminated entirely. Converter: drop `smart_time`; if it read FALSY on the old tag (absent, `''` or `'0'` — the pre-1.6 renderer read `! empty()` and GB merged no defaults, so all three showed midnight) and the tag produces time output, inject `showMidnight` (bare key) to preserve old "always show midnight" behavior. Same ERA gate as `omit_current_year` above, and the same reason ([#90](https://github.com/davidofchatham/bws-gb-dynamic-tags-extensions/issues/90)). NB a hand-edited `smart_time:false` is TRUTHY under `! empty()`, so it drops without injecting — GB never serializes a false boolean, so that spelling only ever reached the wire by hand. **The rename and converter shipped in 1.6.0; the UNGATING did not land until 1.19.1 ([#125](https://github.com/davidofchatham/bws-gb-dynamic-tags-extensions/issues/125)), during which this row sat here reading as complete.** The internal bool it became is now `hide_midnight` and governs midnight suppression alone. |
...
| *(external)* | *(its own prefix)* | Current post (the integrator's own context) | Template as-is | external | External; historical name — the modifier prefix was renamed in v1.6.0, and that generation itself deleted by its owner on 2026-09-08. No prefixed generation of this family registers anywhere now; the owner keeps a registered chain root instead |
