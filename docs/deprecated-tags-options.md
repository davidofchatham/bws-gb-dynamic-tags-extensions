...
| *(no predecessor)* | chain step `entries,<field>` | `{{table}}` (step 6); legal wire elsewhere | New vocabulary (1.17.0) | Promotes a repeater to a traversal step. A `try_`/join slot holding one resolves the step and renders nothing, because no arm on either container assembles a repeater row — which is also why `entries` is on no step offer. |
