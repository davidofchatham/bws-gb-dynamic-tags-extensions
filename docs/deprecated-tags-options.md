...
**No family is registered for this migration today.** bws-portal-system 5.9.0 (2026-09-08) deleted its `view_*` family AND its `bws_register_modifier_root_migrations()` companion in the same release, so a `{{view_title}}` still stored in content neither renders (nothing registers the name — GB emits the literal string) nor converts (no entry describes the rewrite). That is the owner's decision about the owner's names; recorded here because the rows below now read as a worked example rather than as a conversion this plugin can still perform. Retiring the API these rows describe is `future-work.md` FW-129.
