...
| `rel` | `ref1` | `second_related_post_*` deprecated wrappers | Approved (never implemented) | First-hop reference field key. Numeric counter because `ref` type repeats. Supersedes intermediate rename `1st_rel`. **Moot as of 1.17.0** — `SecondRelatedPost::resolve_id()` is inert (#56), so nothing reads `rel`/`rel_2` under either spelling; the family stays registry-only with no migration target. A second-degree hop is a two-step chain, not a numbered option pair. |
