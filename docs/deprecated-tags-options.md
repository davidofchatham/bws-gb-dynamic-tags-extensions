...
**No EXTERNAL family is registered for this migration today.** bws-portal-system 5.9.0 (2026-09-08) deleted its `view_*` family AND its `bws_register_modifier_root_migrations()` companion in the same release, so a `{{view_title}}` still stored in content neither renders (nothing registers the name — GB emits the literal string) nor converts (no entry describes the rewrite). That is the owner's decision about the owner's names; recorded here because the rows below now read as a worked example rather than as a conversion an external plugin can still perform. Retiring the API these rows describe is `future-work.md` FW-129.
...
| `srcTermIn` (slug) | step `terms,<tax>` inside the `src` value | `srcTermIn` / `N-srcTermIn`, on any tag | Implemented (v1.17.0) | **Second rename for this key** (`srcTerm`+`tax` → `srcTermIn` → a chain step), and a shape change like the `refs` row above, so a migrator must handle both eras. A legacy `srcTermIn` is APPENDED to a chain that has no `terms` step of its own — it is a separate option KEY describing a step, so dropping it would lose a configured step, and appending it beside an existing one would double-hop. **A SITE root never takes the legacy term step:** the pair is hand-edit-only (`srcTermIn` registers `show_if src: not:site`) and every arm has always let the site read win, so appending there would flip a stored tag from "the site value" to empty. The `bws-term-hop` control that authored the flat key is **gone as of v1.21.0** (FW-67), along with the families that never took a chain source and so kept it alive (`term_` and `{{table}}`; `view_` was the third until its owner deleted it in portal 5.9.0, 2026-09-08). Nothing offers the flat key any more; the three processing allowances for stored wire — the compiler's appended `terms` step, the fold migration's conversion, and the slot-fold control's deletion of a stale control — are unaffected. |
...
| `portal` | `portal_` | Current post (portal context) | Template as-is | `bws-portal-system` | External; historical name — modifier prefix renamed `view_` in v1.6.0, and the `view_` generation itself deleted by its owner in portal 5.9.0 (2026-09-08). No prefixed generation of this family registers anywhere now; the `view` chain root is what survives |
