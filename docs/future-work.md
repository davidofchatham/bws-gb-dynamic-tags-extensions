...
Progress: Accepted as a regression, not hidden: the old behaviour worked only because a WooCommerce product id happens to equal a post id — the same coincidence that hid the term-id leak (#123, closed) — so an unrecognized shape must say nothing ([I15]). WooCommerce 11.1.0 landed on the fixture site 2026-09-14, and the snapshot baseline was re-captured to absorb its chrome — measured there: it moves all 19 pages and no rendered tag among them. **Product fixture and matrix rows landed 2026-09-14** (blueprint v21, `/matrix-products/`, `loop-test-matrix.md` §QLP): three seeded products, a real `WooCommerce_Query` loop, and the refusal reproduced end to end for the first time. The item shape was measured off that run rather than off the extension's source, and it is **26 keys, not the two the 2026-08-26 source read recorded** — `sku`, `price_html`, `stock_status`, `add_to_cart_url` and others sit beside `id` and `name`. `loop-item-classify-test.php` §C1.13 still pins only the refusal, against a hand-built record.

Open: What marker identifies a product record. The 26-key measurement widened this rather than settling it — an item-side marker is now available where the two-key record offered none, so the candidate list is (a) post-arm widening gated on resolved post type and (b) an item-side key, weighed against those keys being the extension's vocabulary rather than WooCommerce's. Whatever ships must stay SHAPE-keyed, never vendor-keyed.

Blocked by: decision:what marker identifies a product record  •  Interacts with: FW-97 (discharged — Woo's chrome was already in the baseline and the new page added a snapshot file while moving no existing one, measured 2026-09-14), [I15]
