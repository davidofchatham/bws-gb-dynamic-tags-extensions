...
`src(rows,<field>)` resolves to a repeater ROW, and the base-tag arms that read one are landing family by family. `{{text}}`, `{{content}}`, `{{email}}` and `{{phone}}` read one since 1.21.0; `{{image}}` and the two `{{datetime_*}}` families still fall through their dispatch and render empty. Unimplemented wire, not inert wire (`CONTEXT.md` §Language).
