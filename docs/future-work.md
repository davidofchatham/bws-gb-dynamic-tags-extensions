...
Blocked by: row:FW-74  •  Interacts with: FW-8 (independent — see above), FW-74
...
`src(rows,<field>)` resolves to a repeater ROW, and the base-tag arms that read one are landing family by family. Every base family that can read a keyed sub-field reads one since 1.21.0 — `{{text}}`, `{{content}}`, `{{image}}`, `{{email}}`, `{{phone}}` and both `{{datetime_*}}` tags — and the step is on every chain-step offer since the same release. What is left is a fixture row for a genuine NESTED repeater.

Detail home: `.scratch/repeater-row-arm/spec.md`

Progress: The read layer already carries a live `case 'meta_row'`; only the arm is missing. Scoped 2026-09-04 — reach, the read seam's decomposition, and the four provenance keys the producer records are all settled in the detail home, as is the one design rule (branch on the WIRE kind, never `$base['kind']`) and the mutation that proves it. **The seam's decomposition landed 1.21.0** (ticket 01): `bws_read_resolved_source_value()` returns what the store holds, `bws_read_resolved_source()` is the string coercion over it with its signature unchanged, and the array-preserving post read is `bws_read_field_preserving_arrays()`, which the meta-image getter now shares rather than owning. That lifts FW-7's gate as a side effect (see its row). **The producer's four provenance keys landed 1.21.0** (ticket 02): a row produced by a `rows` step carries `parent_kind`, `parent_id`, `repeater` and `index`, stamped by `bws_pipeline_rows_to_sources()` off arguments `bws_run_step()` already held — nothing consumes them yet, and FW-3 is what will. **The TEXT arm landed 1.21.0** (ticket 03), and with it the whole new mechanism: `bws_base_sources_of_kind()` is the sources-not-ids selector (the ids one maps over it), the `try_` arm table's `meta_row` row gained the only id-less PLURAL combination (`ids: 'sources'`, `fn: 'row'`, `list: true`, `branchable` still false), and `{{join}}` came free through the text absorb seam. Analogs refuse on a row by decision, so `use:title` renders empty there. **The remaining SCALAR-keyed families landed 1.21.0** (ticket 04), and it was two thirds the size the ticket predicted: `{{email}}` and `{{phone}}` read rows through `bws_resolve_field_values()`, which dispatches nothing on kind and hands every source to the L2 seam, so their BASE tags already worked and were measured doing it before any code moved — only their `try_` halves, which dispatch per arm, needed a `try_row_fn`. `{{content}}` needed a real arm and its landing FIXED a wrong-entity leak: a `rows` chain fell into the post route, the collapsing selector's empty-fan leg read the ambient post, and `{{content src:rows,…}}` printed the whole surrounding page. Its row read COLLAPSES rather than fans, because the arm takes the family's own cardinality (`takes_first_usable`) rather than inventing one. **The IMAGE arm landed 1.21.0** (ticket 05), and it is the family the read seam was split for: an image sub-field's stored shape follows its own field's return format, so ACF's default hands the row an ARRAY and the string seam drops it — `bws_row_custom_image_core()` reads through the RAW seam and hands the result to `bws_process_meta_image_value()`, the same processor the post route reaches through `bws_get_meta_image_data()`. Collapsing like `{{content}}`, `use:featured` refusing like every other analog, and the stated fallback emitted at the arm rather than in the core, so a `rows` chain keeps the post route's two fallback occasions. The blueprint gained the fixture that can tell a row read from the old leak: three image sub-fields covering all three ACF return formats, on two rows carrying DIFFERENT attachments, neither of them the page's own. **The two DATETIME arms landed 1.21.0** (ticket 06), which completes the keyed families: two separate families with two separate refusal call sites, both fanning (neither is `takes_first_usable`, and both keep their `sep`), and both consuming the VALUE half of the read only — the field-config object id stays each core's own derivation, so a wire row parses FORMAT-AGNOSTICALLY and FW-3 is what closes that. The boundary is seeded rather than described (`review_dmy`, §F9.5m4: one ambiguous row value that reads as the wrong month beside one that cannot be misread), so the day FW-3 lands the row moves and says so.

**The step OFFER landed 1.21.0** (ticket 07): `rows` is on the base tag's, `{{join}}`'s and `try_`'s chain-step lists, which `control-order-test.php` §7 asserts equal, so it could not have landed on one alone. The ticket's "not on the modifier or view families" criterion had no referent left — both families were withdrawn before it ran. Two things came with the offer rather than before it, each because nothing could author the step until now: the argument picker (`bws_fold_rows_picker_def()`, one definition both builders read, pre-scoped to repeater fields with its filters left visible), and the preview's `Rows '<field>'` segment. The engine's input-kind list needed no widening — it already accepted `rows` off every kind including `meta_row`, which is what makes a nested repeater offerable a step at a time.

Open: the field-object read a wire row needs for ACF return formats is FW-3's, not this — datetime on a wire row SHIPPED format-agnostic, and §F9.5m4 is the seeded row that shows what that costs. A fixture row exercising a GENUINE nested repeater is ticket 08, and until it lands `fold-test-matrix.md` §F9c.4 stands in for it.
...
Detail home: `.scratch/plans/field-selector.md` (v2/v3/v-future + the FU follow-ups + the shipped v1 record); in-flight KIND AXIS `.scratch/field-selector-kind-axis/spec.md` (the unscoped-group kind-binding + the `refs`-tail post-type narrowing, plus a §Findings section recording what went stale in the long-lived plan)

Progress: v1 shipped 1.13.0 — ACF + sub-fields + options-page + term-meta + registered meta, flat searchable list with two filters, kind/group-dynamic label, free-text + clear, a REST endpoint inlined per editor load, offered-iff-resolvable. One scope nibble landed with FW-39 in 1.20.0: a SPECIFIC term or post narrows the list to that taxonomy's or post type's own fields plus any group not tied to one location, and re-narrows when a different one is selected. That is the root-argument case only; the `refs`-tail post-type narrowing closed separately in 1.21.0, below.

Progress, 1.21.0 — the **Filter-1 preset nibble**, taken opportunistically alongside FW-74's repeater arm because both rewrite one derivation. Three parts: the preset now follows the CHAIN rather than the legacy flat keys (the old reading was `srcTermIn` plus a literal `src === 'site'`, so the only term preset sat on wire that has not been authorable since 1.17.0 while `terms,<tax>` presetted nothing); a chain ending on a repeater presets to that repeater's own path and names it in the control label; and a `rows` step hands its successor's field picker the repeater it resolved to, so a nested repeater's picker offers that repeater's sub-repeaters instead of every repeater on the site. Both kind maps ship from `bws_fold_wire_vocabulary()` on `window.bwsChainKinds`, so a step type or root added in PHP presets with no editor edit. A declaring root deliberately presets the LABEL only, leaving the FW-39 scope narrowing to own the list. Pinned: `field-combo-control-test.js` §F14/§F15, `slot-fold-repeater-test.js`, matrix §M13/§M14.

Open: v2 type-priority (recommend-divider or multi-select Filter 2; `ref`→relationship+post_object; smarter Filter-1 preset — the derive-from-chain half landed 1.21.0, what remains is last-choice memory and derive-from-the-current-key-value; dynamic label on `ref`; custom combobox widget for reopen-highlight); v3 Pie Calendar; v-future pick-a-post-to-scan.

Closed 2026-09-18, immediately after the kind-binding below and in the order that ticket fixed: **a `refs` tail narrows its successor's picker to the stepped-through field's allowed post TYPES.** Reworded from "`src:ref` stepped-to-PT scope" before it was taken — that named wire nothing has been able to author since 1.17.0, the same defect class as the Filter-1 preset bug fixed earlier in 1.21.0. The editor genuinely cannot derive the type, which is what the old exemption rested on; the ENDPOINT can, because an ACF relationship or post object declares its allowed `post_type[]` in its own config, so the types are stamped on each discovery record as `ref_types` and read back off the record the argument names. Third instance of a shipped pattern: the same additive, machine-readable stamp `repeater_key` takes for a `rows` step. Read LIVE, never serialized into the wire, so a reconfigured field cannot go stale. An unrestricted field and a field discovery never saw both narrow nothing — LOOSE is the honest answer where the type genuinely varies per target, and the wrong-KIND case below is what "wrong" is reserved for. `predecessorContext()` and the fold's own read-picker context were widened to hand `refs,<field>` over, which also removed the last place the fold answered differently from the base tag on identical wire. Pinned `field-combo-control-test.js` §F16 (mutation-checked), `field-discovery-test.php` §ref_types, `slot-fold-repeater-test.js`, matrix §M15 + M11.3 + M12.7.

Closed 2026-09-18 (surfaced 2026-09-17), taken FIRST, ahead of `{{table}}`'s column pickers: **an unscoped field group is offered only under its own KIND.** Changes behavior shipped in 1.20.0. Why the presumption favored the endpoint is in the commit body; the user-facing effect is the 1.21.0 CHANGELOG. **§F13.7 did NOT move** — the claim carried here before said it would, and it covers a failed resolve, where the narrowing stands down before kind is consulted. **Unblocks the second thing:** the Location preset stands down for a declaring root (see below) because the two narrowings disagreed about kind; they now agree, so the preset could apply there. Not taken here.

Closed 2026-09-17, in the same pass: **a declaring root presets the LABEL, never the Location filter.** The gate first read the RESOLVED root argument, which made the filter LOOSEN as the author supplied information — an empty Term source presetted "Term fields", and choosing a term dropped back to "All detected fields", because with an entity in hand the scope narrowing took over and that narrowing spans kinds (the Open item above). Now the gate reads what the root IS, so one source gives one answer in either state. The kind still reaches the label. Pinned `field-combo-control-test.js` §F15.4/§F15.4b, matrix M14.7.

Closed 2026-09-17, in the same pass: **a `refs` tail presets `post`.** It had been the one slug exempt from the preset since 1.13.0 (`22bddf1`), on the ground that a `refs` argument names the field stepped THROUGH and so the post it lands on has an unknown type. That objection is about post TYPE and the preset states KIND — `refs` produces `post` unconditionally, the engine forces it and `BWS_FOLD_STEP_KINDS` records it — so the exemption withheld a fact the render was already committed to, and left the author a list holding term and site fields a post read cannot reach. No revisit trigger is owed here: the preset is derived from that map, so a change to what `refs` produces carries automatically. What would need revisiting is `refs` producing MORE THAN ONE kind (a relationship reaching a term or a user), and that fails the single-valued map before it reaches the editor. The post-TYPE half closed in the same release, one item up, off the field's own config rather than off the wire. Matrix M11.1 is reversed in place and says so; `field-combo-control-test.js` §F15.5 pins it.

Open, added 2026-09-18 out of the refs-tail review: **the pool should be bound to whatever kind the chain resolves to, wherever that is known at parse time — not to `post` at one hard-coded site.** The refs-tail narrowing binds its pool to kind `post` with a literal, which is correct and narrow: `refs` produces `post` unconditionally, so every `refs` tail can gate. Every OTHER step type whose produced kind is known at parse time could gate the same way and does not — a `terms` tail resolves to a term and still leaves post and site records in the pool, hidden only by the Location preset, which is a starting VIEW that widens back. The general rule would read the kind off `window.bwsChainKinds`, the same map `presetKind()` already reads, so there would be no second spelling of it and the `'post'` literal would go. **What makes this a decision rather than a tidy-up is the declaring-root boundary.** `presetKind()` answers for a declaring root too, and gating there would narrow an unresolvable `term,999` to term-kind — exactly the case where "narrowed to nothing" and "this taxonomy has no fields" are indistinguishable, which the kind-binding item below deliberately protected (§F13.7). So the general form needs the same stand-down the Location preset already carries for a declaring root, and that is three rules in conversation. The boundary is already guarded: widening the gate to any tail fails eleven rows across `field-combo-control-test.js` §F13/§F15/§F16, §F13.7 among them.
...
Blocked by: —  •  Interacts with: FW-14, FW-20, FW-131 (a specific user would want the same narrowing a specific term and post got), FW-132 (a record-side read would want its own keys offered from somewhere too)
