...
Open: The term-archive branch only, and what it costs is six CALL SITES, not this function. Deleting the branch means the families entering on a falsy id reach the term arm through the factory instead — `bws_resolve_base_source()` then `bws_read_resolved_source()` in place of `bws_read_field()` — at [content-tags.php:56](../includes/tags/content-tags.php#L56) and [:201](../includes/tags/content-tags.php#L201), [image-helpers.php:247](../includes/helpers/image-helpers.php#L247) and [:250](../includes/helpers/image-helpers.php#L250), [email-tags.php:297](../includes/tags/email-tags.php#L297), [phone-tags.php:564](../includes/tags/phone-tags.php#L564), and datetime's pair at [datetime-helpers.php:341](../includes/helpers/datetime-helpers.php#L341)/[:349](../includes/helpers/datetime-helpers.php#L349). Datetime has already done exactly this once (1.15.0, `bws_datetime_coerce_read_target()`), so the shape is shipped, not speculative.

**The gate is FW-74, and it is a code fact rather than an ordering preference.** `bws_read_resolved_source()` returns `string`; the image family needs raw arrays (`$single_only = false`), and FW-74 is the item that splits out a `mixed`-returning `bws_read_resolved_source_value()`. Until it lands, image cannot move, and the branch cannot be deleted while any caller still needs it — so moving the other five buys churn with no payoff. Re-typed from `decision:cores take a resolved base` on 2026-09-16.
