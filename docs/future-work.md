...
| FW-36 | Deprecated vs Removed settings split (tags AND options) | FW-1's removal made Keep/Suppress/Disable a dead no-op for all 108 tag-type entries (GB registration now unconditionally empty) — but settings page still shows them as if live. Same live/inert axis applies to option-type entries too: an option migration is "removed" once the runtime code's legacy-key fallback (e.g. a `$options['old_key'] ?? $options['new_key']` chain, per the `key`/`rel` B2 pattern) is itself deleted — at that point the migration is the only path back, pure historical record, not an active safety net. Plan: FOUR boxes — "Deprecated Tags" (K/S/D-meaningful; empty today, placeholder for a future round of tags that stay registered) + "Removed Tags" (inert, list-only; holds all 108 today) + "Deprecated Options" (fallback still live in code; holds all 11 today) + "Removed Options" (fallback deleted; empty today, placeholder). Liveness is a hand-set fact per entry, not derived — tags: presence of a `callback` key (existing); options: new explicit flag (e.g. `legacy_fallback_removed`) set when the alias code is actually deleted, mirroring how V7's empty-body `bws_register_deprecated_tags()` is itself hand-maintained. Reposition whole 4-box + Migration Tool group above Diagnostics. Also scans all content on activation/upgrade (+ existing "Scan All Content" button) and hides zero-match entries until rescanned (cuts noise), with a diagnostic "show all" toggle (Diagnostics area) that bypasses the hide filter as a permanent safety net. Needs its own SPEC after FW-1 ships. | `row:FW-1` (ship first) | FW-1 | — (plan not yet written) |
