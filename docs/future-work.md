...
| FW-47 | Author-kind permalink + image analogs | The 1.15.0 author kind shipped `title`/`content` only; `{{permalink}}`/`{{image}}` on an author archive render empty (honest gap) pending two design calls. **permalink** — datum EXISTS (`bws_resolve_link_url` already resolves `get_author_posts_url`), so shipping is trivial; the open question is whether it's WORTH it: the author-archive permalink is circular (it's the URL of the page you're already on), unlike term/post permalink. **image** — no clean intrinsic analog (parity with the #29 term-image gap): the avatar (`get_avatar_url`) is the candidate but adds an external Gravatar HTTP + privacy surface and isn't "featured-image" semantics; a `use:key` ACF user-image field already works (key-mode, not analog). Decide per tag; datetime author analog folds in with FW-9's datetime work, not here. | `decision:permalink-circularity + image-avatar-analog` | FW-9 | `.claude/plans/context-aware-base-tags.md` §Tag Dispatch (author rows) + `bws_base_user_analog_read` PHPDoc (the two deferred cases) |
