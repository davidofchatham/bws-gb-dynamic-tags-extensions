...
| Smart field selector — discovery-backed combobox replacing GB `key`/`ref`/datetime-key text inputs (ACF + sub-fields + options-page + term-meta + registered meta; grouped, creatable, type-to-search, kind-dynamic label). **Source-aware field scope**: resolved-source KIND axis (post/term/site — editor-knowable half of L1) + sub-scope auto-tracking sibling `src`/`ref`/`srcTermIn`, always overridable (GB's blind spot = current-post-only). REST `bws-dynamic-tags/v1/fields`, registration-time not value-time. v1 discovery + scope; v2 type-priority + ref hopped-PT scope; v3 Pie Calendar; v-future pick-a-post-to-scan. **Decoupled from the `use:key,field` combine — no wire change.** Grill-hardened 2026-07-02. | Planned (v1) | `.claude/plans/field-selector.md` |
| Field-selector post-v1 follow-ups (ultra review 2026-07-06): **FU-1** location filter carries kind/group as structured data, not a parsed display string (fixes `' › '` delimiter forgery + locale-fragile kind inference; G2 parenthetical-strip already band-aided); **FU-2** `bws_field_key_option()` factory for the ~14 hand-copied option flips (fold into v2). Both low-severity, v1 ships without them | Deferred | `.claude/plans/field-selector.md` §Post-v1 follow-ups |
