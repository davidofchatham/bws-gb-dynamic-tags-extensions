...
Detail home: `docs/design-history/multi-step-slot-sources.md` §What FW-43 keeps; build-locality decision `docs/design-history/combine-text.md` §Build locality

Progress: Shipped 1.18.0 as `bws_read_bounded_sources()` (field-helpers.php), extracted from try_'s emit loop; both the content/permalink/image term loops and the try_ slot emit consumed it. FW-136 (1.21.0) deleted that emit, so the consumers since are the base collapsing reads alone (`bws_base_post_first_usable()`, `bws_base_term_first_usable()`, the content and image resolve seams), which a `try_` attempt now reaches through its base seam; the selector's own harness is `tools/test/read-bounded-sources-test.php`. The content callback's post-side collapse to one id is confirmed intentional (`{{content}}` is not list mode because a value carries no identity of its own — an assembly decision, not a resolver one; recorded on #118 (closed), a separate editor-surface concern). The #108 (closed) coverage gap is closed — the arm wiring it named was pinned under mutation until FW-136 deleted it, moving every `try_` attempt onto its base tag's resolve seam.

Open: Un-hardcoding the `same`-use prepend. (The second open clause, that the shared `try_` emit had no pure-harness coverage, dissolved with FW-136: the emit is deleted, and the attempt walk that replaced it is pinned by `tools/test/try-slot-loop-test.php`.)

Blocked by: —  •  Interacts with: FW-49 (closed; the combining half), FW-71 (closed), FW-136 (closed; deleted the `try_` emit that consumed this)
...
Progress: Traced 2026-08-29 — a bare `{{text}}` on a term archive re-parses the chain 3x across 9 files / 14 hops; a folded `try_` slot re-parses 5x per slot, including once on a string the seam had just emitted. That `try_` count was traced through the pre-1.21.0 closure, which FW-136 deleted; an attempt now reads through its base tag's resolve seam, and the count on that path has not been re-traced. Does not re-open ADR 0002, which governs L1's resolved SOURCE (a binding); this concerns the CHAIN (a parse result).
...
Blocked by: —  •  Interacts with: FW-113 (removes dispatch sites that would otherwise each need threading), FW-107 (closed), ADR 0002 (scope, not conflict)
...
Progress: `make_modifier_callback()` in `class-tag-template-registry.php` (part of `register_modifier()`; the `$base_kind` ternary) reimplements term-vs-post dispatch inline instead of calling the seam — but `$base_kind` there is a binary ternary off `get_context_type()`, whose interface contract is closed to `'post'`/`'term'` codebase-wide (every implementer, including `CallbackRoot`, coerces to that pair), so the seam's `user`/`query_context` arms are structurally unreachable from this call site. Pure DRY/unreachable-arm cleanup, not a bug. The two datetime families call the seam (in `bws_base_datetime_single_resolve_value()` / `bws_base_datetime_range_resolve_value()` since FW-136 moved them out of the callbacks) but only for link-identity/kind-branching, not value — no reader case exists for `datetime_single`/`datetime_range`, so the seam's `value` is always `''` there and is discarded, never compared; this is partial, intentional adoption (identity via seam, value via the term core), not a re-decision that could diverge. The `case 'user':` carve-out in `bws_base_ambient_analog()` remains a measured, deliberate (tag × kind) guard, not a defect. Filed as one item because the two remaining sites share a root cause (nobody calls the seam), not because either is broken.

Open: Neither remaining site should be fixed standalone. Site 1 sits inside `make_modifier_callback()`, ORPHANED as of 1.21.0 — `register_modifier()` was its only caller and now mints nothing (FW-129) — so it is dead code awaiting the 1.22.0 stub delete, and joining it to the seam would be work on a function already scheduled for deletion. Site 2's two resolve seams are the exact ones FW-81 collapses into one — joining the seam there before FW-81 means redoing the join on the merged one. Expected to resolve for free, site 1 with FW-129's 1.22.0 delete and site 2 with FW-81, rather than needing its own PR — re-check after either lands.

Blocked by: —  •  Interacts with: FW-47 (widening the user arm dissolves the third, already-excluded site), FW-33 (closed), FW-3, FW-106, FW-129 (site 1 is dead code inside the withdrawn `register_modifier()` and goes with its 1.22.0 delete), FW-81 (site 2's seams are the ones it merges — sequence after), FW-136 (closed; moved site 2 from the callbacks into the resolve seams, verbatim)
...
Detail home: this item, plus `bws_base_ambient_analog()`'s PHPDoc (the enforcing site) and its census guard in `tools/test/traversal-pipeline-test.php`. Commit `95984b9` (2026-09-02) found and fixed the two live instances and is their record. No `.scratch/` plan exists because the structural version was never started, only proposed and turned down for that session's scope. (Until 2026-09-23 this line pointed at a private memory note; its content duplicated this item and was folded in here and into the PHPDoc.)

Progress: Two per-tag carve-outs shipped 1.19.1 (`base-shared.php`'s `user` and `query_context` cases, `image` only). A census guard (`traversal-pipeline-test.php`, reads the seam's own `case` labels off disk) pins that both existing carve-outs stay present, so a THIRD ad hoc copy of the same fix doesn't ship unnoticed the way the second one did. `content`/`datetime_single`/`datetime_range` fixed the same session at their own callback tails — not testable through this seam's return value at all, since their fix isn't IN the seam; their regression protection is the page-snapshot baseline instead (`context-test-matrix.md` C-C2/C-DT1/C-DT2). Since FW-136 (1.21.0) those three tails sit in each family's resolve seam (`bws_base_<tpl>_resolve_value()`) rather than its callback, carried verbatim and re-measured on C-C2/C-DT1/C-DT2 unchanged. Every `try_` attempt now resolves through that same seam, so this seam's per-tag carve-outs govern `try_` output as well, where the old arm path bypassed it.
...
Blocked by: decision:structural-vs-per-tag  •  Interacts with: FW-113 (its two remaining sites duplicate the seam's dispatch question without calling it — a different defect, same function; its own `user`-carve-out row confirms the carve-out pattern this item's per-tag fixes use is deliberate, not a defect), FW-136 (closed; took this contract as a fixed input and widened its reach to `try_`)
...
Blocked by: decision:whether the option is useful at all  •  Interacts with: FW-87 (closed), ADR 0007 (§Why the read-based axis was reversed), [I19]
...
Open: Whether `try_` is structurally "base + N slots" or carries real structural difference — answered on the RENDER side by FW-136 (1.21.0): the inline resolve is deleted, and a `try_` tag is its base tag's resolve seam plus `bws_try_run_attempts()`, the walk. What remains to check is the REGISTRATION side (the per-slot options and their repeater); the add-slot control (largely answered by FW-45's repeater spike); the slot-noun label ("Add fallback" rejected — collides with the shipped `fallback` option).

Blocked by: `decision:absorb try_ into base at all`  •  Interacts with: FW-57 (closed), FW-45, FW-27, FW-43, FW-24, FW-33 (closed), FW-136 (closed; standalone from this by decision, and it leaves no renderer work here — it also answers open question 1 above as a build artifact rather than a research task)
...
| FW-125 | Consume GB 2.4's dynamic-data trust model at the surfaces GB cannot reach | Shipped 1.19.2 (2026-09-10): one seam asks GenerateBlocks whether the current user may author dynamic data (`bws_gb_user_can_author_dynamic_data()`, with the key-scoped `bws_gb_option_allowed_for_current_user()` beside it), and two per-user gates ride it: a site option value no longer resolves in the editor preview for a user who cannot author dynamic data, and the field-key picker's data is no longer sent to one. The untrusted-user REST fixture both gates still lack went to FW-127. The item left the tracker in `274be94` without a ledger row; this row was added 2026-09-23 | CHANGELOG 1.19.2 §Security; `includes/helpers/gb-trust-boundary.php` PHPDoc; ADR 0008; `docs/design-history/dynamic-data-trust-predicate.md` (the build record); `verify.php`'s GB-trust-model section |
