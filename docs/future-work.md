...
| FW-36 | Deprecated vs Removed Tags settings split | FW-1's removal made Keep/Suppress/Disable a dead no-op (GB registration now unconditionally empty) — but settings page still shows it as if live. Plan: split into two categories — "Deprecated Tags" (future families that stay registered, K/S/D stays real) vs "Removed Tags"/"Migration-only Tags" (GB registration gone, informational list + Tag Converter only, no mode control). Needs its own SPEC after FW-1 ships. | `row:FW-1` (ship first) | FW-1 | — (plan not yet written) |
