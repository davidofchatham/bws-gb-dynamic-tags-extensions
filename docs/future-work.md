...
Open (2026-09-11, must be decided before 1.20.0 ships): whether the `term_ tags` toggle should still gate term-context SOURCES. Seeding it off on new installs — the deprecation's own mechanism — also removes the `term` chain root from the base tag and slot source enums, because `is_source_enabled()` answers that toggle for every term-context source. That is the root FW-39 pins and the root the conversion writes into. Measured; two resolutions and a recommendation are in the detail home, ticket 09.
