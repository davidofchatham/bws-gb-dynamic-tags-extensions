...
Open: v2 type-priority (recommend-divider or multi-select Filter 2; `ref`→relationship+post_object; `src:ref` stepped-to-PT scope; smarter Filter-1 preset — the derive-from-chain half landed 1.21.0, what remains is last-choice memory and derive-from-the-current-key-value; dynamic label on `ref`; custom combobox widget for reopen-highlight); v3 Pie Calendar; v-future pick-a-post-to-scan.
