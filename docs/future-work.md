...
Progress: Not started. Five sites enumerated and the reachable ones identified (the `{{text}}`/`{{content}}`/`{{title}}` user arm is live via `bws_base_user_analog_read()`); GB's own gating read in 2.4.1 and recorded in `docs/gb-constraints.md` §Dynamic data is suppressed by POST SOURCE.
...
**Also left to this item: whether `register_modifier_template()` stays an integrator surface at all.** Since FW-136 a registered template yields only a `try_` tag, and only when it names a base tag's resolve function as `resolve_fn`, so an external caller has to run a base tag of its own to get anything out of it. `docs/plugin-integration.md` §4 Option A still offers the route; its errors were corrected in place in 1.21.0 (a broken example that set none of the keys the constructor reads), and the cut was left here.
...
Progress: Designed 2026-09-22 by grilling; every decision is settled in the detail home. **Ticket 01 landed 2026-09-22** — the attempt walk is out of the closure as `bws_try_run_attempts()` (`includes/helpers/try-slot-loop.php`), taking the family's resolve callable as a parameter and therefore harnessed without WordPress for the first time (`tools/test/try-slot-loop-test.php`); the `resolve_fn` descriptor key is in, the eight un-flipped families stand on `TagTemplateRegistry::try_arm_resolver()` (the old arm dispatch, moved verbatim behind the parameter), and `text` is the first family across. Measured on the flip: exactly ONE row moved across the whole page-snapshot baseline — `L4a.2` on `/matrix-terms-valid/` and `/matrix-terms-mixed/`, a multi-term `try_text` attempt that now prints an anchor per value instead of none, which is FW-135 on the first of its four families. **Ticket 02 landed 2026-09-22** — `email` and `phone` flip onto `bws_base_email_resolve_value()` / `bws_base_phone_resolve_value()`, the first two of the five families that register no link options and therefore the first test of the prediction that those five move nothing. They moved nothing: all 23 snapshot pages matched the committed baseline unchanged, and an 891-cell `render-tag` sweep (81 email/phone tag strings harvested from the matrices and the fixture blueprint × 11 contexts, 198 of them rendering non-empty) was byte-identical between the two builds. Neither family carries a `try_query_fn`, so the query-context question below stayed open rather than being answered here. **Ticket 03 landed 2026-09-22** — `permalink` flips onto `bws_base_permalink_resolve_value()`, the first COLLAPSING family across and the thinnest shell of the nine (this family registers no `fallback` and emits no preview label, so the seam's value is the whole return). It moved nothing either: all 23 snapshot pages matched the committed baseline, and a 392-cell `render-tag` sweep (28 permalink-family tag strings × 14 contexts, 202 rendering non-empty) was byte-identical between the two builds. The collapse rule is not restated in the seam — the walk bounds the attempt to one result off `takes_first_usable`, and the seam reads no `limit` at all, so a slot-stated, tag-level or carried number cannot reach it. This family carries no `try_query_fn` either, so the question below is still open after three tickets. **Ticket 04 landed 2026-09-23** — `title` flips onto `bws_base_title_resolve_value()`, the SECOND of the four link-registering families and so the second place FW-135 dissolves. It moved output, and only where it was designed to: a 672-cell `render-tag` sweep (48 title/`try_title` tag strings × 14 contexts, 402 rendering non-empty) differed in exactly 12 cells between the two builds, every one of them a `try_title` whose attempt FANS and which sets `linkTo` — joined-and-unlinked before, an anchor per value against each value's own entity after. The question below is answered for this family, and is the first family to answer it: the base route DOES reach the query context (`bws_base_ambient_analog()` claims the kind and `bws_base_query_context_analog_read()` carries a `'title'` case), so `title`'s `try_query_fn` descriptor was DELETED with the flip rather than left to the final commit — it was never a per-entity core, only the arm shim's route to that context. Nothing is owed to FW-9. Measured after the delete: `{{try_title}}` and `{{title}}` render identically on all five query-context ambients (search, date archive, 404, post-type archive, latest-posts home). The visible half is new: LK.5 and LK.6 on `/matrix-links/` are the `try_` twins of LK.2 and LK.4 (`fold-test-matrix.md` §F9b F9b.4b/F9b.4c), each measured equal to its base twin byte for byte on the new build and joined-and-unlinked on the old one, so the baseline diff is four added lines on one page. Designed measurements that day: only four of the nine `supports_try` families register link options (text, title, datetime_single, datetime_range), so FW-135 is observable on four and the other five cannot be half-done on that axis; and the fold's `sep`, bound and limit-default handling already agree byte-for-byte with `bws_try_join_items()` plus `bws_read_bounded_sources()`, so those five are expected to move no output at all. The base half of the split already exists on one family — `bws_base_text_resolve_value()` has been the shell/seam shape since 1.14.1 — which is what makes the seam a generalization rather than an invention. FW-135, FW-107 and FW-43's open residue dissolve when it lands rather than being fixed. **Ticket 05 landed 2026-09-23** — `image` flips onto `bws_base_image_resolve_value()`, the widest of the five families that register no link options: the `as`/size read, the Media Library fallback and the array-preserving repeater-row read all sat in the prologue being split. It moved nothing. All 23 snapshot pages matched the committed baseline, and a 1022-cell sweep (73 image/`try_image` tag strings × 14 contexts, 119 rendering non-empty) was byte-identical between the two builds. The fallback axis took a second sweep of its own, because a stated Media Library fallback is a seeded attachment id no matrix string can name (`fold-test-matrix.md` F11b.3's standing exception): 252 more cells over 18 fallback-carrying shapes, 222 of them non-empty, also byte-identical. Where this family's split PARTS from text's, and the whole of its difficulty: the stated fallback stays in the SEAM rather than moving to the shell, because the image cores emit it per read and the refusal arm emits it without one, while a logo-less `src:site` and an empty term fan print nothing at all. One shell tail over all four would have made the last two print the fallback — a defensible alignment, and out of scope for a flip whose deliverable is zero movement; the refusal arm keeps its own preview-outranks-the-fallback ordering in the seam for the same reason (it is the one arm where both can apply). This family carries no `try_query_fn` to retire and the base seam does not claim the query-context kind for image either, so both routes keep falling through to the post arm and nothing is owed to FW-9. **Ticket 06 landed 2026-09-23** — `content` flips onto `bws_base_content_resolve_value()`, the last of the three collapsing families and the messiest prologue in the set: the callback returned a preview label at three points MID-resolution (the site arm, the refusal arm, the ambient arm) before it ever reached a value, and all three had to become an empty read the shell could put one label on. It moved nothing. All 23 snapshot pages matched the committed baseline, and a 924-cell sweep (66 `content`/`try_content` tag strings × 14 contexts, 398 rendering non-empty) was byte-identical between the two builds — then run a SECOND time with `bwsEditorPreview` set, where all 924 cells render and 558 of them differ from their front-end twin by carrying a label, also byte-identical. That preview half is what pins the two arms this family could not simply hand to the shell: the REFUSAL arm is the one place where the label and the stated fallback can both apply, so it keeps its own preview-outranks-the-fallback ordering inside the seam (image's refusal arm, same shape), and the AMBIENT arm keeps FW-116's per-tag fix verbatim — it terminates in preview and falls THROUGH on the front end, and only the preview flag separates the two. Nothing was found reading wrong under the seam's contract, so nothing is recorded on FW-116. The query-context question is answered for this family, the same way title answered it: the base seam reaches that context itself (`bws_base_ambient_analog()` claims the kind for every tag but image, and `bws_base_query_context_analog_read()` carries a `'content'` case), so the `try_query_fn` descriptor was DELETED with the flip and measured after the delete — `{{try_content}}` equal to `{{content}}` on all five query-context ambients. Nothing is owed to FW-9. **Ticket 07 landed 2026-09-23** — `datetime_single` flips onto `bws_base_datetime_single_resolve_value()`, the THIRD of the four link-registering families and the largest extraction in the set. It moved output, and only where it was designed to: a 2,436-cell `render-tag` sweep (174 `datetime_single`/`try_datetime_single` tag strings × 14 contexts, 528 rendering non-empty) differed in exactly 34 cells between the two builds, every one of them a `try_datetime_single` whose attempt FANS and which sets `linkTo` — joined and unlinked before, an anchor per value against that value's own entity after. Every one of the 34 is its old value plus anchors, byte for byte, so no date string, format token or timezone read moved; no base-tag cell moved at all. The sweep was run a SECOND time with `bwsEditorPreview` set and reports the same 34 cells and nothing else, once the CLOCK the preview label prints is masked (the label embeds the current time, so two sweeps a minute apart differ on 525 cells for a reason that is not a build difference — a trap worth knowing before reading a datetime preview sweep). Where this family PARTS from title's, and the reason its FW-116 tail could not go where the ticket said: the stated fallback stays in the SEAM. The cores emit it themselves on a read that found nothing, and the compensating tail exists for exactly the arms where no core ran — which arms those are (fanning, refused, ambient-claimed) is a fact only the seam holds, so a shell could not have expressed it. It is inert under `try_` regardless, because `bws_try_run_attempts()` strips `fallback` before calling the resolver. The FW-116 tail is that tail's `null !== $ambient` term, carried verbatim and re-measured: `context-test-matrix.md`'s C-DT1 renders `TBA` on all six of its contexts on the new build, and its snapshot pages did not move. This family carries no `try_query_fn` to retire, but the query-context question still got a NEW answer here rather than title's: the base seam CLAIMS the kind (`bws_base_ambient_analog()` excludes only `image`) and `bws_base_query_context_analog_read()` has no datetime case, so a flipped `try_datetime_single` answers empty one layer EARLIER than the post arm it used to reach. Same empty, different route — `fold-test-matrix.md` F19.4 restated to say so, its value re-measured unchanged on both builds. Nothing owed to FW-9, and nothing found reading wrong under the seam's contract, so nothing recorded on FW-116. The visible half is new: LK.7 and LK.8 on `/matrix-links/`, the base/`try_` datetime pair (§F9b.4d). They earn a third pair on that page because LK.5's values ARE each entity's own title, so a build taking the link from the VALUE rather than from the source would still render LK.5 correctly; a date says nothing about where it lives. Baseline diff is that one page. **Ticket 08 landed 2026-09-23** — `datetime_range` flips onto `bws_base_datetime_range_resolve_value()`, the FOURTH link-registering family and the ninth and last flip: every `supports_try` template now carries a `resolve_fn` (9 of 9), so nothing resolves through `TagTemplateRegistry::try_arm_resolver()` any more and ticket 10 can delete it. It moved output, and only where it was designed to: a 518-cell `render-tag` sweep (37 `datetime_range`/`try_datetime_range` tag strings × 14 contexts) differed in exactly ONE cell — the fanning `try_datetime_range` that sets `linkTo`, joined and unlinked before, an anchor per whole range against that range's own post after, and the after value is the before value plus anchors byte for byte. One cell and not ticket 07's 34 because the corpus holds exactly one range shape that both fans and links; the other 517 are the parity evidence, the start/end pair, date inheritance and every format token among them. The sweep was run a SECOND time with `bwsEditorPreview` set — all 518 cells render a label there — and reports the same single cell once the CLOCK is masked, the trap ticket 07 recorded, widened here because an `H:i` label prints a bare `11:35` with no meridiem to key on. The FW-116 tail is carried verbatim and re-measured: `context-test-matrix.md`'s C-DT2 renders `TBA` on all six of its contexts. No `try_query_fn` to retire, and the query-context question gets ticket 07's answer again — the base seam CLAIMS the kind and `bws_base_query_context_analog_read()` has no datetime case, so the empty arrives one layer earlier by a different route; measured empty on all five ambients on both builds, `fold-test-matrix.md` F19.4 restated to name both datetime families. Nothing owed to FW-9, nothing found reading wrong under the seam's contract, nothing recorded on FW-116. The visible half is new: LK.9 and LK.10 on `/matrix-links/` (§F9b.4e), the base/`try_` range pair, earning a fourth pair on that page because a range is TWO reads already joined by `rangeSep` before the fold sees the value — the one row where the per-item wrap goes around an assembled pair. Baseline diff is four added lines on that one page. **Ticket 09 landed 2026-09-23** — FW-135 closes, dissolved rather than fixed, and the author-facing disclosure written against it on 2026-09-22 is retired in all three places before ever reaching a release: the URL Meta/Option Field Key help text goes back to the sentence it carried before, and `docs/tag-reference.md`'s `linkKey` row and `linkTo` prose now state the `try_` behavior as the base one, per value where an attempt fans. The 1.21.0 per-item-link entry loses its `try_` carve-out and names a `{{try_*}}` attempt alongside the four tags, which is what it now does; the merge itself takes no entry of its own, because within one unreleased version a refactor that moves no user-visible behavior is not a delta. The five families that register no link options are deliberately NOT mentioned to authors: they have no such setting to be told about. **Ticket 10 landed 2026-09-23** — the arm path is deleted: `TagTemplateRegistry::try_arm_resolver()`, the arm table (`includes/helpers/try-slot-arms.php`) with its harness, `bws_try_join_items()` and `bws_try_normalize_items()` with theirs, every `try_*_fn` descriptor key, and the walk's third resolver argument, which existed for the arm path alone. `resolve_fn` stays as the one descriptor key a `try_` tag requires. It moved nothing: a sweep of 494 `try_` tag strings harvested from the matrices, the fixture blueprint and every earlier ticket's corpus × 14 contexts (6,916 cells, 4,156 rendering non-empty) was byte-identical between the two builds, and again with `bwsEditorPreview` set (6,300 non-empty) once the preview clock is masked; all 23 snapshot pages matched the committed baseline. Two sections of the deleted arm-table harness were never about the table — the site read chosen by resolved kind rather than token spelling, and the census against wire-string `src` compares — and moved to `slot-fold-test.php` §P19 rather than going with it. FW-107 closes in the ledger, dissolved rather than fixed.

Answered at ticket 08 (all nine families flipped): every `try_query_fn` descriptor that existed is deleted, `title` and `content` because their base seams reach the query context themselves, and nothing was owed to FW-9 — the six families that never carried one answer the same empty by the base route. The rule stands for any family added later: where the base route does not reach that context, that is an FW-9 gap on the base tag and is filed there rather than kept alive as a `try_`-only arm.

Blocked by: —  •  Interacts with: FW-135 (dissolved 2026-09-23 and closed in the ledger — the per-item link wrap is inherited from the fold, not threaded through the emit), FW-107 (dissolved 2026-09-23 and closed in the ledger — the lifted loop IS its stated fix shape), FW-43 (its open residue is the emit this removes), FW-116 (its contract is a fixed input; three families carry its fix in the callback tail being split), FW-92 (NOT absorbed — registration-side editor work, unchanged by this), FW-60 (standalone from this by decision; after the merge FW-60 has no renderer work left in it), FW-81 (will find two datetime seams rather than two callbacks), FW-9 (where a query-context gap surfaces)
...
Also settled 2026-09-22: the fixture seeds email obfuscation OFF, because `antispambot()` flips a coin per character and an obfuscated `{{email}}` therefore differs from ITSELF between two renders of one build — which makes a before/after sweep report differences that are one address under two encodings. Measured that day: all 23 snapshot pages pass with the toggle in EITHER position (the normalizer decodes ASCII numeric references once), so moving the fixture default costs no baseline. Obfuscation is a state to switch ON for a targeted check and switch back, never the state a full sweep runs in; `src-site-test-matrix.md` §R6.9 is the row that wants it, and it reads in that direction now. The PRODUCT default is unchanged and still ON.
...
| FW-4 | `src:site` slot for the remaining `try_` tags | Shipped 1.15.0 — pure wiring: a `try_site_fn` descriptor leg, five thin closures over `bws_site_resolve_value`, single-result site link-wrap for I6/C9 parity | CHANGELOG 1.15.0; registry PHPDoc (`try_site_fn`, deleted by FW-136); `src-site-test-matrix.md` R7 |
| FW-5 | Collapse the `try_core_fn`/`try_term_fn` fork | Retired 2026-08-15 by #103 (closed) — the four hand-written `try_` arms collapsed onto one dispatch keyed by resolved source kind, through the pure table in `includes/helpers/try-slot-arms.php` (both deleted by FW-136) | FW-71 |
...
| FW-107 | The try_ slot resolver has no seam under it | Dissolved by FW-136 rather than fixed: the arm table and the closure that consumed it were both deleted once all nine `supports_try` families read through their base tags' own resolve seams, so the refusal the post-arm branch overwrote no longer has a table to be stated in. The stated fix shape is what shipped — per-attempt resolution lifted into a named function, `bws_try_run_attempts()`, taking the family's resolve seam as a callable rather than a string another file switches on — and `try_query_fn`'s shape asymmetry dissolved with it, since no family carries one | FW-136 (what dissolved it); `bws_try_run_attempts()` PHPDoc + `tools/test/try-slot-loop-test.php` |
