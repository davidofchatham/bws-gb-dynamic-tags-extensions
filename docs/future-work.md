...
| FW-45 | join dynamic slot count | Drop the fixed `BWS_JOIN_MAX_SLOTS` (8) for an add-slot editor control (arbitrary N). The constant is threaded through resolve loop / token scan / option emit / reveal chain so the cap raise itself is one change; the CONTROL is the blocker. | `code:custom editor-control work (docs/editor-controls.md reserved owner)` | FW-24 | `.claude/plans/archive/combine-text.md` §Slot count |
...
| FW-22 | `{{join}}` tag | Shipped 1.15.0 (standalone combining tag, 8 text slots, separator + template modes, %N wire tokens). Spawned FW-43/44/45/46. | CHANGELOG 1.15.0; `tag-reference.md` §join; `tools/test/join-test-matrix.md` + `join-template-test.php`; plan archived `.claude/plans/archive/combine-text.md` |
