...
Open: The likely outcome is deletion, not rename — the carrier dies with `register_modifier()` (FW-129) or with `{{table}}` taking a chain source (FW-53). Also open, added 2026-09-07: the carrier's taxonomy list is PUBLIC-ONLY, while the entity picker (FW-39, shipped 1.20.0) lists every REGISTERED taxonomy behind a capability check, so a private editorial taxonomy is now selectable and still not hoppable. The divergence was accepted rather than reconciled — reconciling it changes a shipped control's offering on every existing tag, and this carrier is slated for deletion here.
