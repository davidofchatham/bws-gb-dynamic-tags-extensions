...
| FW-47 | Author-kind permalink + image analogs | The 1.15.0 author kind shipped `title`/`content` only; `{{permalink}}`/`{{image}}` on an author archive render empty (honest gap) pending two design calls. **permalink** — datum EXISTS (`bws_resolve_link_url` already resolves `get_author_posts_url`), so shipping is trivial; the open question is whether it's WORTH it. On the author archive itself a bare `{{permalink}}` IS circular (the URL of the page you're on) — and the non-circular uses don't reach this path: a multi-author loop resolves each row as a POST (loop-row wins over user-ambient, so it's post permalink there), and `src:ref`→user isn't wired. So the author permalink analog only earns its keep once a NON-ambient user source exists (`src:ref`→user, or the ID source FW-39 pointing at a user) — better gated on FW-32/FW-39 than decided standalone. **image** — no clean intrinsic analog (parity with the #29 term-image gap): the avatar (`get_avatar_url`) is the candidate but adds an external Gravatar HTTP + privacy surface and isn't "featured-image" semantics; a `use:key` ACF user-image field already works (key-mode, not analog). Decide per tag; datetime author analog folds in with FW-9's datetime work, not here. | `decision:image-avatar-analog` (permalink soft-gated on a non-ambient user source, FW-32/FW-39) | FW-9, FW-32, FW-39 | `.claude/plans/context-aware-base-tags.md` §Tag Dispatch (author rows) + `bws_base_user_analog_read` PHPDoc (the two deferred cases) |
