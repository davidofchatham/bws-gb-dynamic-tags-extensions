...
Open: v2 type-priority (recommend-divider or multi-select Filter 2; `ref`→relationship+post_object; a `refs` TAIL narrowing its successor's picker to the stepped-through field's allowed post types — reworded 2026-09-18 from "`src:ref` stepped-to-PT scope", which named wire that has not been authorable since 1.17.0: flat `ref` survives only as a `flatAxes` migration axis and `bws_wrapper_ref_steps()` reads `refs`-typed steps off the assembled chain, so the old spelling described a rule nothing could satisfy — the same defect class as the Filter-1 preset bug fixed in 1.21.0. Designed at `.scratch/field-selector-kind-axis/` ticket 02, ordered strictly after the kind-binding below, timing otherwise open; smarter Filter-1 preset — the derive-from-chain half landed 1.21.0, what remains is last-choice memory and derive-from-the-current-key-value; dynamic label on `ref`; custom combobox widget for reopen-highlight); v3 Pie Calendar; v-future pick-a-post-to-scan.

Open, surfaced 2026-09-17: **an UNSCOPED field group is offered under an entity of another KIND, and the endpoint that emits `scope` says it should not be.** `bws_field_discovery_derive_kind_scope()`'s own docblock states that a group with no kind-bearing location rule "returns kind `post` with empty scope … an empty scope means 'any post type' client-side" — kind-bound. The consumer says the opposite ("A field with no scope of its own stays offered under either kind"), so a selected TERM is offered post-kind fields today; `field-combo-control-test.js` §F13.1/§F13.2 pin that as the shipped answer. **The clause is under-specified rather than decided:** `git log -S"under either kind"` puts it in `86d858f`, the commit that built the narrowing, whose body says only "plus every unscoped one" and is silent on kind, and FW-39's spec never mentions unscoped at all. The endpoint owns what its own field means, so the presumption favors kind-bound — but flipping it changes what every selected term and post offers, which shipped in 1.20.0, and §F13.1/13.2/13.3/13.7's lists move with it (F13.2's RULE survives; its example stops being cross-kind). It lands here because this item already owns what the list offers. **Doing it also settles a second thing:** the Location preset stands down for a declaring root (see below) precisely because the two narrowings disagree about kind; once they agree, the preset can apply there with no collision. **DECIDED 2026-09-18 — kind-bound, and it goes FIRST**, ahead of `{{table}}`'s column pickers: a column picker anchors on its container chain's resolved tail, so the defect would land once per COLUMN rather than once per tag, and deciding afterwards would move `{{table}}`'s matrix rows a second time. Build design `.scratch/field-selector-kind-axis/` ticket 01; the §F13.7 half of the claim above is flagged there as probably wrong (it covers a failed resolve, which kind-binding does not obviously touch) and is to be confirmed rather than carried forward.
