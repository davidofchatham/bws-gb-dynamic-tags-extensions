...
| `src:site` slot support for `try_text` / `try_title` / `try_content` / `try_image` / `try_permalink` (today only `try_email`/`try_phone` set `try_allow_site_slot` — they route their `try_core_fn` through the SEAM `bws_resolve_field_values`, which has a site arm; the other templates route through post/term cores which are site-blind). Reachable ONLY when those templates' try dispatch rides a kind-aware seam that reads site — but `content` (body/excerpt), `image` (attachment), and `title`-analog are NOT value-list reads, so this is gated on the SAME seam expansion (content/image/analog arms) as the datetime-seam row above. Surfaced during Phase 1 T8. NOT reachable by the `try_core_fn`/`try_term_fn` fork collapse alone. | Likely | `.claude/plans/traversal-pipeline.md` §Post-Phase-1 convergence |
| Collapse the `try_core_fn`/`try_term_fn` fork (paired post-core/term-core dispatch fns per template) into ONE kind-dispatching try handler — DEFERRED from Phase 1 T8 (T8 scope narrowed to term-ambient parity, the reachable I6/C9 fix). Pure structural cleanup (deletes ~7 paired `bws_try_*_post_dispatch`/`bws_try_*_term_dispatch` fns), high V8 byte-identity risk in the densest callback, zero behavior gain on its own. Best done AFTER the seam expansion (above) proves the unified kind-dispatch shape — the collapse target is a seam-routed handler, not a hand-merged leaf-fn pair. | Likely | `.claude/plans/traversal-pipeline.md` §Post-Phase-1 convergence |
