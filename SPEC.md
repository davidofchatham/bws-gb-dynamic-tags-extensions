...
- `includes/tags/fn-tags.php` — NEW.
  - `bws_register_call_function( string $fn, array $meta = [] ): bool` — sugar over raw `add_filter`. Runs the security gate at registration (fail-fast `_doing_it_wrong`); `function_exists` → false + warn; `isInternal` → false + warn; else `add_filter('bws_fn_passthrough_functions', fn($l) => $l + [$fn => $meta])`. `$meta` accepted (forward-compat), UNUSED v1. Raw `add_filter` path STILL works (power users / bulk).
...
| T3 | x | `bws_register_call_function( string $fn, array $meta = [] ): bool` — registration sugar. `function_exists` false → `_doing_it_wrong` + false; `isInternal` true → `_doing_it_wrong` + false; else `add_filter('bws_fn_passthrough_functions', fn($l) => $l + [$fn => $meta])` + true. `$meta` stored, unused v1. Raw `add_filter` path documented as still-valid. | VC-gate,VC-allow,I.fn-tags |
...
| T9 | . | Docs: `tag-reference.md` registry row + I4 source-level gate row + 4th-structural-position note; `CONTEXT.md` 4th-position note (AT SHIP only); `editor-tag-previews.md` inert-exception note; `CHANGELOG.md` `### Added`; `README.md` "for developers; bring your own function" overview. | VC1,VC2,VC-inert,VC-empty,I.tag-reference |
| T10 | . | Version bump (plugin header + `BWS_DYNAMIC_TAGS_VERSION` const + readme stable tag). Minor bump from 1.11.0. | — |
| T11 | . | Regression matrix: `src:current` loop-row resolves post id; `src:ref` hop; allowlisted fn returns string verbatim/unescaped; non-string/empty → fallback; `arg:` non-empty passed pos-1, absent → fn default; bucket-A (stale ref / fn gone / hand-edited builtin) → fallback + editor warn; #6 throw → `error_log` + fallback + no message leak; empty allowlist → no options; raw `add_filter` parity with `bws_register_call_function`; inert preview never executes. | VC1,VC2,VC3,VC-arg,VC-gate,VC-fail,VC-inert |
