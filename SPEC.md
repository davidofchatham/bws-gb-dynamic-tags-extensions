...
T7|~|Phase 5: slot-resolver site arm added (registry, after srcTermIn before post path) — `src:site` short-circuit, `$cf(0,...)`, no link-wrap. Per-template re-allow: `try_allow_site_slot` flag → `bws_build_slot_traversal_options($n,...,$allow_site)` 4th param skips #26 site filter. text/image stay filtered (no flag). LIVE-PENDING (email/phone templates land Phase 6/7)|V7,V8
T8|~|Phase 6: `email` template registered via `bws_register_email_template()` (called in bws_register_base_tags before term_ pass + try_ gen). key+subject+noLink+fallback opts, visibility gate, per_slot_key, no use enum, no-key-values=[], list_options, allow_site_slot. Shared `bws_email_finish_values` (validate+render-one); base callback refactored to reuse it (V10). try_/post/term dispatchers built. lint+harnesses green. LIVE-PENDING editor+frontend|V10,V12,I
T9|~|Phase 7: `phone` template twin via `bws_register_phone_template()`. Shared `bws_phone_finish_values` (cc/stripCc once, render-one); base callback refactored to reuse. dispatchers + cores built. lint+harnesses green. LIVE-PENDING|V10,V12,I
T10|x|Phase 8: preview email/phone — `bws_build_try_preview_label` needs_key=true branch + template_label (Email/Phone) + field_part. #24 fold: empty→`⚠ slot N no key`. Harness 62/62 (5 new email/phone cases)|V12
T11|~|Phase 9: term_email/term_phone fall out of register_modifier (term_fn/post_fn present in template). Base preview handles via prefix-strip (email/phone cases pass). LIVE-PENDING register+render verify. NOTE: term_ src:site wrong-reads (make_modifier_callback has no site arm — pre-existing term_*-wide, see §B1)|V13
