...
| T6 | . | Inject link options + wrap into **`register_modifier()`** (term_ tags): read `supports_link_wrap` from template descriptor; append options to leading group; wrap `term_fn`/`post_fn` return with entity-type-aware call | V4,V7,V8,V9,I.taxonomy-tags |
| T7 | . | Inject link options + wrap into **`generate_base_try_tags()`**: read `supports_link_wrap`; append `bws_get_link_options()` to `$options` after leading group; at closure return-point (lines ~665, ~672) capture entity type + id from winning slot and call `bws_wrap_with_link()` | V5,V7,V8,V9,I.registry |
