...
T6|~|Phase 5: `bws_resolve_field_values` extracted to field-helpers.php (verbatim shared L1/L2 body, function_exists-guarded). Both clones `bws_email_resolve_addresses`+`bws_phone_resolve_numbers` DELETED; email/phone callbacks call shared. lint+all harnesses green; phone-normalize unregressed. LIVE-PENDING base {{email}}/{{phone}} byte-identical (clone-faithful copy, but WP-runtime smoke needed)|V9,C8
