...
**Query context as a read source** (I1 applied by context; five entity-less kinds, 1.19.0): on a date archive, a post-type archive, search results, a 404 or the latest-posts home a bare base tag reads the context's own analog — `title`→core's document-title branch for that context (archive label, date span, search heading, the GP 404 borrow, site name), `content`→the post type description on a PTA and the GP 404 text borrow, `text use:title`→the same heading as `title`; `image` has no query-context analog either, but — like the term column above — still reaches the post route for a configured Media Library fallback rather than rendering empty; everything else renders empty, never `$post`'s value. The resolved source carries sub-kind + payload and no id (PTA's `queried_object` is a `WP_Post_Type` with `queried_id` 0 — captured 2026-07-18), so link identity is null (I12). A `try_` slot resolves identically (I6) on the same three tags; the other six families render empty. Explicit sources and query-loop items win, as in the term and user columns. The per-kind option surface (search format, 404 override, home title-source) stays deferred on FW-9. Detail: `tag-reference.md` §Context coverage; decisions of record in `.scratch/plans/context-aware-base-tags.md`; rows `tools/test/context-test-matrix.md`. *[This paragraph replaced a "still Phase 2 … falls through to `$post`" clause on 2026-08-29: the kinds shipped first (`2188c96`, PR #129) and that change's doc sweep missed this file, so the doc moved to describe what shipped rather than the code being wound back — the decision itself had moved, in the grilling session the plan records.]*

**AMBIENT IS `current`, AND IT IS KIND-AGNOSTIC; AN ENTITY-KIND ROOT THEREFORE TAKES AN ARGUMENT** (1.20.0, FW-39). Everything above is what ONE root means: `current`, which is also what an absent `src` stands for, and which answers with whichever kind the four rules land on — post, term, user or query context. No second root can mean "whatever the page is about", because that meaning is already taken and is already kind-free. So a root named for an entity KIND has only one question left to answer, WHICH ONE, and it answers it from an argument the author supplies (`term,34`, `post,1692`); a bare `term` or `post` root names no entity and resolves nothing rather than collapsing back into `current` under another spelling. This is why the two are not alternative ways to say the same thing and why neither replaced the other when both shipped. The argument's own grammar, the argless policy and the offering rule are schema: [`tag-reference.md` §Root enum membership](docs/tag-reference.md#root-enum-membership-1170-83). What an argless declaring root DOES is [I15] at the root layer, owned at the factory seam; what the steps off a root with an argument may do is the engine's step-input table's, owned there.
...
Single-class detail is PHPDoc on the enforcers: `bws_resolve_base_source` / `bws_capture_ambient_signals` (precedence + degenerate-term guard), `bws_run_traversal` (resolved-source typedef, pure fold), `bws_read_resolved_source_value` (kind dispatch + post/0 guard; `bws_read_resolved_source` is the string coercion over it), `bws_pipeline_default_reader` (ACF-compatible ref), `bws_base_ambient_analog` (the one ambient gate, every kind — the term/user twins retired 1.19.0), `bws_base_term_analog_read` (term analog + image fallback), `bws_base_user_analog_read` (author analogs), `bws_base_query_context_analog_read` (query-context analogs, core-following). Schema: `tag-reference.md` §Source-analog resolution, §List mode. Rationale + probe: `docs/design-history/traversal-pipeline.md`.
...
- **Tags** — Removed when `prefix_removed` is set; else the interim default reads `callback`-presence. **`callback`-presence is a proxy, NOT a render guarantee**: post-FW-1 the GB dispatch loop is gone, so a present `callback` (carried by an external alias) no longer means the tag renders. The proxy survives only because our own removed N×M entries had their `callback` stripped (→ Removed) while external aliases still pass one (→ Deprecated) — two populations with opposite natural defaults that a single global-default flag would split wrong. **The second population is empty as of 2026-09-08** (below), so nothing currently exercises the `callback` arm; the proxy is left in place because emptiness is a fact about today's installed set, not about the classifier.

**External context-modifier aliases take their authoritative status from THIS plugin.** An external plugin's alias (one prefix pointing at a newer prefix of its own) is a *modifier over a tag this plugin owns*; its target's live/removed status is authoritative here, and the external additionally owns a `prefix_removed` flag it sets when it retires that prefix generation. Either target-removed OR prefix-removed ⇒ the alias files under Removed. Rebuilding these aliases inside the external plugin is rejected — they are modifiers of our tags, not standalone tags the external owns (revisit only if base tags ever become a drop-in module).

**No external alias exists today, and no new one can be minted.** The one integrating plugin DELETED its fourteen aliases and the nine targets they pointed at on 2026-09-08, rather than flagging `prefix_removed` on them — so the retirement path this invariant describes went unused by its only integrator, and its sole surviving integration is a registered chain root. Then FW-129 withdrew `register_modifier()` in 1.21.0 (a `_doing_it_wrong()` stub minting nothing, deleted in 1.22.0), so the population cannot refill either. **The invariant still BINDS, and is not retired with the API.** `prefix_removed` is a flag on migrator ENTRIES, and the entries outlive the tags by design — the `term_*` ones are kept precisely so stored wire still converts after the family stopped registering. A flag that still rides shipped entries is still a hand-set fact the classifier reads, whatever mints them; and `DeprecatedTagRegistry` prefix aliases were never gated on `register_modifier()` in the first place. What changed is the population's ceiling, not the rule.
...
Kinds with no link identity are **normal, not exceptional**: `meta_row` (a repeater row is not an entity — it has no URL) today; the [I9]/#19 query-context kinds (date archive, PTA, search, 404, latest-home) since 1.19.0, refused by name in `bws_source_link_identity()` — the tracker called them "entity-less ... no field to read", and that is what they are. A new source kind's link obligation is therefore ONE question: does it address something? If not, `null`, and no consumer changes.

**Corollary — a list links PER ITEM, and each anchor spans exactly one entity.** Every collected value is wrapped against its OWN identity, and the `sep` join runs over the already-wrapped parts, so a separator never falls inside an anchor and a value with no identity prints plain beside its linked siblings rather than costing the list its links. That is what the per-value identities are for: linkability is carried by the value, so nothing about how many values survived decides whether any of them can be linked. The **per-item fallback stays suppressed** (GH #51) — the fallback fires once, in the caller, on all-empty output, never per item. A fallback emitted per item would pollute the list with a string that is not a value, and under the count-based gate this rule was first written for it would additionally have satisfied that gate, linking a string that addresses nothing. The gate is gone; the suppression it motivated is not, because its first reason never depended on it.
...
- **A DECLARING root's argument is absent, or resolves to nothing** (1.20.0, FW-39 — `term,<ID>`). Applied at the ROOT layer rather than a new invariant: a root that declares an argument and was given none REFUSES (or answers by a rule its OWNER states — `SourceInterface::ROOT_ARGLESS_OWNER_RESOLVES`, for a root that ships argless today and gains an argument later) — it never falls through to `resolve_id()`'s ambient read. Equally terminal when the argument names something that no longer exists: `term,34` after term 34 is deleted refuses, it does not become the term the visitor is looking at. The axis lives at the factory/root seam, not at the resolving source: the root policy is enforced before `TaxonomyTerm::resolve_id()` is ever reached, and the `term_*` modifier family — which calls that method forever, and does not pass through the seam — keeps reading the ambient term. **This sentence formerly said `resolve_id()` was UNTOUCHED, and it no longer is** (1.20.0): its ambient arm now gates on the queried object's TYPE, which is a different axis from this one — a wrong-entity bug in the legacy ambient read, surfaced by the fixture testbed, not a root-layer refusal. The doc was edited to match the shipped code by decision on 2026-09-10, the direction that normally needs a note and gets one here: the invariant was always about WHICH SEAM enforces the root policy, and naming a file as untouched over-stated it into a claim about that file's whole future, which the root layer never needed and could not keep.
...
**Corollary — a carried source carries what it IS, not merely its root.** `src(same)` names the same *source*, so its steps travel with it: a leading `terms` step leaves the root unset by design, and a slot spelling `same` that took only the root landed on the ambient entity. Contrast `limit`, which is a *parameter of* a source and is container-sensitive for that reason. **HALF OF IT IS STRUCTURAL since [#104](https://github.com/davidofchatham/bws-gb-dynamic-tags-extensions/issues/104)** — the carry holds a chain, and a chain holds its own hops, so the `$tax_inherit` scalar that carried the hop is gone. The DISPLACE half is not structural and is still a rule: part of the carried chain can GIVE WAY to a step this slot states of its own, and the ROOT never does, because a slot carries over the SOURCE. It is a rule about what `same` means — a slot sentinel the container resolves before compiling — not about the chain grammar, so a base tag's `terms,a;terms,b` still hops twice.
...
- **A carried source carries its steps for free.** With a chain-shaped carry, [I15]'s corollary stops needing a special case: `src(same)` copies a chain, and a chain holds its own hops.
...
**Corollary — offering is STATED, never derived, and stays that way permanently.** The registry keeps its dead by policy: `register_source()` is never deleted for lacking resolve logic, so four retired traversal-substitute classes remain registered right now. None of them promote to the root enum, because `bws_registered_root_rows()` is the ONE appender both authoring surfaces — the base tag's root enum and the folded slot's source enum — take their rows from: "offered here, absent there" cannot happen, and a dead-by-policy registration cannot leak into either dropdown just by existing. **`term` and `post` both left this list in 1.20.0 (FW-39, tickets 02/03)** without breaking the rule they illustrate: a bare `term`/`post` root is still unofferable (it is exactly what a bare base tag already does — [I15]'s ambient reading), but each now separately offers a ROOT ARGUMENT (`term,<ID>`, `post,<ID>`), stated on the source contract (`SourceInterface::get_root_argument()`) rather than inferred, per this invariant's own opening sentence.
...
## I19 — A limit bounds USABLE sources

A `limit` — a step's, or the tag-level key — bounds **usable** sources: candidates that pass the source gate ([resolvable] × [exists] × [visible], §Language). A source failing the gate never spends budget; an EMPTY READ of a usable source spends its slot and outputs nothing. Field population is no part of the test, so selection is FIELD-INDEPENDENT: adjacent tags on the same source path always read the same entity, and `limit(1)` on a refs step means the first stored reference, only. Every step's limit is the same kind of bound, applied per input by the engine after the gate; the tag-level key is the one GLOBAL slice (a preserved holdover — ADR 0005). The gate's deciding rule is owned at its enforcing site, the traversal engine's gate predicate PHPDoc (traversal-pipeline.php); this invariant states its consequence. `usable` and the limit schema it governs are owned at [`tag-reference.md` §List mode](docs/tag-reference.md#list-mode-limit--sep).

The collapsing tags (`content` / `permalink` / `image`) ignore every limit at every position and read the FIRST usable source, outputting its read even when empty ([tag-reference.md §Collapsing tags](docs/tag-reference.md#collapsing-tags-first-usable-source)). The first-POPULATED search this branch briefly carried was reversed for determinism and survives only as a dormant opt-in seam — decision history in ADR 0007's "Why the read-based axis was reversed".
...
**A retired number is never reissued.** Records elsewhere cite invariants by number, and those citations are not repointed (`docs/cross-link-rules.md` — a design-history record naming a dead path is the record working). Reusing a number would silently redirect them at an unrelated rule, so the next invariant is I21.

- **I20 — `term_*` is kind-LOCKED; a base tag is kind-AGNOSTIC.** Live 1.20.0–1.21.0. Retired with the family it compared against (FW-129); it said so itself. The text, and what the retirement did and did not change, are in [`docs/design-history/term-family-kind-lock.md`](docs/design-history/term-family-kind-lock.md). The capability difference it records still governs the migration entries, which outlive the family.
...
- **detected** — an ambient signal supplies the entity, so it varies per render: WP query object / query-loop item (`src:current`, term-archive), a related-post step (`src:ref`), or the active Site View / user session (`src:view`). `src:view` is detected-yet-explicit — detection is NOT the same axis as implicit/explicit.
- **global** — no per-entity read; a site-wide datum (`src:site`). A Site View may ALSO act site-wide, but `src:view` stays **detected** because a signal (the active view) selects it; `src:site` consults nothing.
- **ID** — the author identifies ONE specific entity and its id is serialized into the token (probable `src:<type>,<ID>` shape, **not final**). The **ID source** — the only flavor carrying a serialized entity id. This is the "specific resource" concept the qualifying gate points at (FW-39 ID source, FW-33 `term_` deprecation). Names the mechanism (serialized id) = the provenance (author supplies it).

Grid: `implicit`→bare queried (detected). `explicit`→ detected (`current`/`term`/`ref`/`view`) | global (`site`) | ID (`src:<type>,<ID>`). Prefer **global** over "fixed" for `site` — "fixed" also fits ID sources (fixed-per-render), so it under-discriminates. _Avoid_: "contextual"/"context source" as the NAME of this axis-2 pole — say **detected** (the pole spans query, a `refs` step, AND session/view; "context" in the doc means specifically the #19 *query*-context, a subset, so naming the whole pole "contextual" blurs it with that subset). "context modifier"/"context-aware"/"context kind" elsewhere are unaffected. Also avoid "entityless" for `global` (collides with unresolvable-read / post/0 empty).

**Author register for an ID source — SELECT a SPECIFIC entity; "pin" is retired here** (ruled 2026-09-15, while FW-39 was still unreleased). The verb an author performs is **select** (they select a term from a picker); the adjective naming the flavor is **specific** ("a specific term", "a specific source"); **fixed** is allowed as a plain-English gloss ("the same term on every page") but never as the flavor's NAME, because it fits `global` equally and so discriminates nothing — which is the same reason the Grid above rejects it for `site`, read from the other end. "Selected" stays barred as the flavor's NAME by the axis-1 rule above; verb yes, name no. The canonical model name is unchanged: **ID source**. _Avoid_ across the whole entity sense: "pin", "pinned", "pinning", "unpinned", "repin" — **`pin` is reserved to its OTHER meaning, an assertion that holds a behavior so it cannot drift** (`CLAUDE.md`'s axis-ownership doctrine, "pinned by `<harness>`", "a harness may name an axis it mechanically PINS"), and the two senses were living in the same files. INTERNAL identifiers for the mechanism take the stem already shipped as public API rather than a new coinage — **`root_arg`** and **`argless`** (`SourceInterface::get_root_argument()`, `ROOT_ARGLESS_OWNER_RESOLVES`, the `arg` key on a `bws_dynamic_tags_chain_roots` spec): "a pinning root" is **a root that takes an argument**. The author register and the identifier stem are deliberately different words for the same thing — author terms never name the wire ([`docs/editor-tag-previews.md`](docs/editor-tag-previews.md) owns the author-facing strings).
...
**Resolvable / exists / visible** (properties of a resolved source — three of the four levels a candidate passes on its way to output; the fourth, *a non-empty read*, deliberately gets no noun because the phrase is already precise):
...
- **exists** — the bound names a LIVE entity. VIEWER-INDEPENDENT, which is the whole reason it is its own level: existence is checkable with no viewer in hand and permission is not, so the two are enforceable atomically at the engine gate (split out of *visible*, 2026-08-21). A trashed post EXISTS; what it fails is *visible*. Which entity kinds are tested, and how, is the gate's own rule — `bws_source_gate()`'s PHPDoc (`includes/helpers/traversal-pipeline.php`) owns it.
- **visible** — the existing entity is one **the current viewer may read**. VIEWER-RELATIVE, deliberately, not a fixed status allowlist: the same author previewing their own draft must resolve it, which only works if the test is relative to who is asking. Enforced with [I19] at the engine's gate predicate, whose PHPDoc owns the deciding rule (decision record [ADR 0007](docs/adr/0007-a-limit-counts-usable-sources.md)); the gate applies at EVERY step, so an entity failing it cannot be a stepping stone — a chain routed through it is cut at that hop.
...
- **Unimplemented** — well-formed wire no arm consumes *yet* (a `rows` step on `{{image}}` or `{{datetime_single}}`, FW-74's remaining families — `{{text}}`, `{{content}}`, `{{email}}` and `{{phone}}`, and with them every slot that absorbs the text read, consume one since 1.21.0). **Unimplemented is not inert**, and conflating them is the mistake this term exists to prevent: it encodes a per-template fact with a shelf life, and the tag becomes correct without anyone touching the sentence that called it broken. The example moving under this definition, and the definition not needing a word changed, is that shelf life arriving.
