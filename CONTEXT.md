...
The SLOT half is still contradicted, and **the mechanism that fixes it is not the one this paragraph used to predict.** A slot's chain is collapsed back to a flat triple before any container arm sees it, so a chain with no flat spelling is skipped rather than run — but the fix is not more kind dispatch in the containers. `{{join}}`'s slot read already dispatches by kind one layer down, and the base reader already accepts a chain-spelled `src`. What has to change is that the seam stops re-spelling: see [I16]. Only `try_`'s arms are a genuine dispatch rewrite. Depth-0 chains resolve today.
...
**NOT YET ENFORCED — this invariant leads the code.** It is recorded here ahead of the build because it is what the build is FOR, and because the contradiction it resolves is already described in [I14]. Enforcement home on ship: `bws_fold_slot_chain_options()` PHPDoc (slot-fold.php), replacing `bws_fold_slot_flat_options()`. Schema: [`tag-reference.md` §Folded slot wire](docs/tag-reference.md#folded-slot-wire-multislot-containers). Rationale + the rejected alternatives: `.claude/plans/multi-step-slot-sources.md`. Related: [I13] (storage is the other axis), [I14] (root is not a step; its slot half resolves here), [I15] (the inherit corollary becomes structural), [ADR 0004].
