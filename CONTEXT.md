...
**Where the two are caught, and why it is not the obvious place.** Both are refused ABOVE the render cores, at the arm. The cores' falsy-id guard looks like it already refuses and does not: a field read with no id does not stop, it falls back to the query-loop ROW and then to the queried TERM. The read beneath that guard is load-bearing — in a flat repeater row an *absent* source legitimately DOES mean the row — so absence and refusal have to part company above the core, because the core cannot tell them apart.
