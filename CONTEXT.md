...
**No external alias exists today, and no new one can be minted.** bws-portal-system 5.9.0 (2026-09-08) DELETED its fourteen `portal_*`/`views_*` aliases and the nine `view_*` targets they pointed at, rather than flagging `prefix_removed` on them — so the retirement path this invariant describes went unused by its only integrator, and the sole surviving portal integration is the `view` chain root (`PortalSource`). Then FW-129 withdrew `register_modifier()` in 1.21.0 (a `_doing_it_wrong()` stub minting nothing, deleted in 1.22.0), so the population cannot refill either. **The invariant still BINDS, and is not retired with the API.** `prefix_removed` is a flag on migrator ENTRIES, and the entries outlive the tags by design — the `term_*` ones are kept precisely so stored wire still converts after the family stopped registering. A flag that still rides shipped entries is still a hand-set fact the classifier reads, whatever mints them; and `DeprecatedTagRegistry` prefix aliases were never gated on `register_modifier()` in the first place. What changed is the population's ceiling, not the rule.
