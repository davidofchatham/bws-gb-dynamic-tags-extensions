...
A `limit` — a step's, or the tag-level key — bounds **usable** results: candidates that survive to output. It counts what will be output, never candidates examined, so a candidate filtered out on the way — not [visible](#language), or an empty read — never spent budget. The deciding rule (collect-then-slice) is owned at its enforcing site, `bws_collect_usable()`'s PHPDoc (field-helpers.php); this invariant states its consequence. `usable` and the limit schema it governs are owned at [`tag-reference.md` §List mode](docs/tag-reference.md#list-mode-limit--sep); [resolvable] and [visible] are §Language's.
