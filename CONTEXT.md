...
- **exists** — the bound names a LIVE entity. VIEWER-INDEPENDENT: post → `get_post()` non-null (a TRASHED post exists — it fails *visible*, for most viewers), term → a valid term, user → a real user, site → vacuously true. Split out of *visible* (2026-08-21) so each level is enforceable atomically at the engine gate — existence is checkable with no viewer in hand, permission is not.
- **visible** — the existing entity is one **the current viewer may read**. VIEWER-RELATIVE, deliberately, not a fixed status allowlist: the same author previewing their own draft must resolve it, which only works if the test is relative to who is asking. Posts get status + capability; terms and users have no publication status, so for them *visible* adds nothing beyond *exists*. Enforced with [I19] at the engine's gate predicate (decision record [ADR 0007](docs/adr/0007-a-limit-counts-usable-sources.md)); the gate applies at EVERY step, so an entity failing it cannot be a stepping stone — a chain routed through it is cut at that hop.
