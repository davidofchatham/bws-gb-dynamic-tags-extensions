...
- **A STEP whose slug is unknown vocabulary**, at any non-leading position. Still contradicted, and it is a DIFFERENT read from the one above rather than the same leak twice: the base arms branch `site` → `term` → `post` and fall to a catch-all singular read with no case for the `''` kind such a chain resolves to, so the unknown step is silently DROPPED and the chain's PREFIX is read, collapsed to its first result. Tracked as [#109](https://github.com/davidofchatham/bws-gb-dynamic-tags-extensions/issues/109). Disjoint from #75 by POSITION — an unknown slug at position 0 is a ROOT and goes there, because a root is not a step ([I14]). Three readers already treat unknown vocabulary as a distinct answer (the compiler's unknown-TYPE rule, `bws_fold_chain_join()`, #105's preview); the arm is the one place it collapses into "no chain", and the guard in `bws_fold_chain_join()` is the posture to copy.
