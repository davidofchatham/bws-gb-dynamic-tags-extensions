...
- **A step `limit` and the tag-level `limit` are different quantities.** The first bounds one fanning step's spread, **per input source**; the second bounds the resolved-source list once, before the read. Neither bounds values — the read is 1:1. A step carries a limit only when it actually bounds the step, since absent is how the engine spells no limit.
