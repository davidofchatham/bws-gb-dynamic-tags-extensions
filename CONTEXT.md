...
**No external alias exists today.** bws-portal-system 5.9.0 (2026-09-08) DELETED its fourteen `portal_*`/`views_*` aliases and the nine `view_*` targets they pointed at, rather than flagging `prefix_removed` on them — so the retirement path this invariant describes went unused by its only integrator, and the sole surviving portal integration is the `view` chain root (`PortalSource`). The invariant still binds: it governs what happens when an external alias exists, and the population is empty rather than the rule withdrawn. Retiring the API that mints these families is `docs/future-work.md` FW-129.
