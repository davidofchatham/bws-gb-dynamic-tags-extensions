...
function bws_register_phone_tag(): void {
	...
	bws_gb_register_tag( array(
		...
		// Source group = src → ref/srcTermIn → sep → key. The phone own-anchor
		...
		'options'    => bws_prepare_registration_options( array_merge(
			...
			array(
				...
				// posts). Scalar sources return one number — hide it. Before the
...
function bws_register_phone_template(): void {
	...
	\BWS\DynamicTags\TagTemplateRegistry::register_modifier_template( array(
		...
		// FW-136 — try_phone resolves each attempt through the BASE seam. The
