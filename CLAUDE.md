...
runs these; run them locally before commit.
   ...
   **Not all PHP any more (1.17.0).** Two harnesses shell out to **`node`** —
   `slot-fold-twin-test.php` and `serialization-order-test.php` (its JS-port twin block) — and in
   both a missing `node` FAILS rather than skips. The pattern below describes the first; the second
   is the same shape at smaller scale (one driver, key lists in, reordered lists out).
