...
`related-post-src-migration-test.php`). Run the one whose domain you touched (see
...
`related-post-src-migration-test.php`. No CI runs these; run them locally before commit.
...
| `rel` → `ref` repair change — the `$rel_fix` foreach (7 base tags), its `term_` sibling loop, or `bws_migrate_slot_rel_to_ref()` (the try_ half), all in `bws_register_option_migrations()` | run `php tools/test/related-post-src-migration-test.php` §R4 + §R5. **WHEN BOTH `ref` AND `rel` ARE PRESENT, THE TAG'S (or SLOT'S) `src` DECIDES WHICH WINS — there is NO ranking of the two keys, and assuming one is the single mistake this area invites.** Each spelling is live under exactly one token and inert under the other: `src:ref` → the compiler reads `ref`, `rel` is inert, ref wins; `src:related_post` → the retired class read `rel` (then `key`) and NEVER `ref`, so rel wins and must OVERWRITE the inert ref; `src` absent → neither was ever read, so no faithful answer exists and it is a repair (keep `ref` if set, else `rel`, inject `src:ref`). Getting it backwards does not error — it migrates a tag to hop somewhere the old tag never hopped, permanently, which is the defect #56 exists to stop. §R5 runs one `ref`+`rel` pair past all three tokens and each must pick a DIFFERENT winner; both directions are mutation-verified. The `related_post` case is settled in the SLOT transform even though a later entry owns that token, because the slot transform runs first and deleting `rel` would destroy the evidence the later one needs. **KNOWN, UNFIXED, SEPARATE: `$rel_fix` itself gets this wrong** — it matches on `rel` presence alone, never looks at `src`, and so overwrites a LIVE `ref` on a `src:ref` tag with the inert `rel` beside it (`{{text src:ref|ref:A|rel:B}}` → `ref:B`, changing which post a working tag reads). Shipped since 1.6.0; tracked separately, do not "fix" it as a drive-by here. **A renamed `ref` MUST arrive with a matching `src:ref`, and the try_ half is a `transform_callback` for exactly that reason.** The first draft renamed keys and injected no `src`, reasoning a bare `N-ref` is inert rather than wrong — it is not inert: the fold entry runs in the SAME cascade, maps a legacy `ref` with no `src:ref` beside it to NO STEP, folds the slot without it and strips the legacy keys, ERASING the relationship in the pass meant to rescue it. §R4.5/§R4.7 are the mutation-verified pins. Two things the declarative pipeline cannot do here: `option_renames` matches an EXACT key (so `2-rel` needs its own pair) and `source_inject` writes the TAG-level `src`, which on a try_ tag is SLOT 1's — a `3-rel` must set `3-src`. **`term_` is derived from the registered TEMPLATES but the `term` PREFIX is hardcoded**, because this runs at init:20 and an external modifier registers later (portal's `view_` is init:21) — a derived prefix list would hold only `term` while promising more; an external plugin registers its own entry. `{{join}}`/`{{table}}`/`{{call}}`/`{{email}}`/`{{phone}}` are excluded: all postdate the 1.6.0 rename, so there is no wire to repair. **POPULATION UNKNOWN BY CONSTRUCTION** — nobody can enumerate what a historic buggy converter wrote; do not read these entries as evidence such wire was found |
...
| ⏳ **TEMPORARY — delete this row when `.claude/plans/src-chain-encoding.md` archives** | TWO plans carry a live §SETTLED index: that one (FW-56/57 + `{{table}}`) and its sub-plan `.claude/plans/per-step-limit.md` (per-hop `limit` semantics + FW-63 arm dispatch), which **WINS on every `limit` question** and records four corrections to rows the master had marked settled. Check the sub-plan FIRST on anything limit-shaped. The general row above is what survives |
