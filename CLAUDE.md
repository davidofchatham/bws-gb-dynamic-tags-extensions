...
| Base-tag name-COLLISION boundary change — anything in `includes/helpers/gb-registration-boundary.php` (`bws_gb_register_tag` = the plugin's ONE registration site, `bws_gb_recheck_tag_ownership` = the late `wp_loaded` re-read that catches the reverse collision, `bws_gb_tags_we_registered`, `bws_gb_tag_name_collisions` = the per-request record both report surfaces read, `bws_gb_report_tag_collision` + `bws_gb_other_registrar_phrase`, `bws_gb_tag_registrar_file`), the `wp_loaded` hook in the plugin bootstrap, or either template constructor's dup-check skip in `class-tag-template-registry.php` | `control-order-test.php` (§9 tree scan + §10 driven collision, both directions) | [rules](docs/update-triggers.md#base-tag-name-collision-boundary-change) |
