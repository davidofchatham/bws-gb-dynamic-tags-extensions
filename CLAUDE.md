...
| Collapsing-capability change — `takes_first_usable` on a template record, the selector (`bws_collect_usable` in `field-helpers.php`) or a consumer swap, the `$ignore_limits` thread (`bws_fold_chain_to_steps` → both assemblers → `bws_base_source_ids_of_kind`), the editor suppression conditional, or the `bws-fanning-advisory` control | `collect-usable-test.php`, `fold-chain-compile-test.php`, `control-order-test.php`, `editor-filter-chain-test.js`, `fold-test-matrix.md` §F15 | — |
