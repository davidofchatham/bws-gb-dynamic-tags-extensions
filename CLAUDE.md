...
| Folded-slot REGISTRATION or RESOLUTION change (`bws_build_fold_slot_options` in `base-shared.php` = the `fold` config every container's control reads; `bws_fold_slot_struct` / `bws_fold_slot_flat_options` in `slot-fold.php` = the render seam; or a container's slot loop — `bws_join_callback`, the try_ slot dispatch, `bws_build_join_preview_label`) | run `php tools/test/slot-options-build-test.php` (derived config) + `php tools/test/slot-fold-test.php` (§P13 render seam) + `php tools/test/preview-label-test.php` (the preview reads through the SAME seam — that convergence is the point, so a seam change shows up there first). **Registration flips WITH the resolver, one container at a time:** modes do not mix (a tag is folded iff any `N:` key exists), so folded keys registered against a flat resolver render nothing. **Carry-forward lives ONCE, in the seam** — three copies (join's loop, try_'s loop, the join preview walk) is what it replaced, and the preview copy had already drifted |
