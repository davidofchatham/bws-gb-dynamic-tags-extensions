...
| Stripped-`use`-default change — `BWS_USE_STRIPPED_DEFAULTS` / `bws_use_stripped_default()` / `bws_use_effective()` = THE read rule, a field token implying the mode (`registration-helpers.php`), the first value of any leaf's `use` enum, or a read site recovering an absent `use` | `use-stripped-default-test.php`, `preview-label-test.php`, `slot-fold-test.php` | [rules](docs/update-triggers.md#stripped-use-default-change) |
