...
| Field-discovery change (`includes/rest/field-discovery.php` transforms, `assets/js/field-combo-control.js`, the enqueue/inline block, a flip of any option to/from `bws-field-combo`, or either half of the pinned-root narrowing — the `scope` slug on `bws_entity_lookup_term_row()` / `bws_entity_lookup_post_row()`, and the `window.bwsRootArgKinds` inline) | `field-discovery-test.php`, `field-combo-control-test.js`, `entity-lookup-test.php`, `slot-fold-repeater-test.js`, `field-selector-test-matrix.md` | [rules](docs/update-triggers.md#field-discovery-change) |
...
| Modifier-family REGISTRATION GATE or GB-TYPE STAMP — the settings gate and the migration-registry `gb_type` read at the top of `TagTemplateRegistry::register_modifier()`, the ORDER of `bws_register_modifier_root_migrations()` against `bws_register_term_modifier_tags()` in the init pass, or the `modifiers` array in `bws_dynamic_tags_activate()`'s seed | `control-order-test.php` §C4, then the testbed for both seed arms | [rules](docs/update-triggers.md#modifier-family-registration-gate-or-gb-type-stamp) |
...
| Converter OWNERSHIP-GUARD change — `includes/helpers/converter-ownership.php` (`bws_converter_tag_ownership` = the pure predicate every content rewrite passes, `bws_converter_rewrite_allowed` = the WordPress-side gatherer, `BWS_CONVERTER_OWNERSHIP_REASONS` = the closed reason enum, `BWS_CONVERTER_OWNERSHIP_OPTIN_OPTION` = the per-name opt-in), `TagConverter::apply_if_owned()` = the ONE place that class asks the guard, or a rewrite loop in `TagConverter::migrate_post()` | `converter-ownership-test.php` | [rules](docs/update-triggers.md#converter-ownership-guard-change) |
...
| Query-loop ITEM RECOGNITION change — `bws_classify_loop_item` and the two shape readers beside it (`bws_loop_item_term_id` / `bws_loop_item_user_id`) in `field-helpers.php`, `bws_loop_item_is_post_or_row` (the predicate six render cores gate on, in place of bare `in_loop`), the shape returned by `bws_get_loop_item_context`, or step 2 of `bws_resolve_base_source` (which maps a recognized kind onto a source kind) | `loop-item-classify-test.php`, `traversal-pipeline-test.php`, `loop-test-matrix.md` rows against the testbed | — |
| AMBIENT-TERM read change — `bws_queried_object_is_term()` in `taxonomy-helpers.php` (THE axis: a queried-object id is a term id only when the queried object is a term), or either site gating on it — tier 4 of `bws_reliable_term_context_detection()`, and the ambient arm of `TaxonomyTerm::resolve_id()` (the `$stated`/`$trusted` pair, which infers WHICH arm of GB's `get_id()` answered by comparing the value). Retires with the `term_*` family (FW-129), not before | `context-test-matrix.md` §C-TERM/CT rows against the testbed, then `php tools/test/page-snapshots.php` — the seven `ctx-*` pages pin every context, and the matrix owns why the rows come in PAIRS | — |
