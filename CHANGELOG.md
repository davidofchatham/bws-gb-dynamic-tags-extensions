...
- **A tag's source path can now follow more than one step.** On `{{text}}`, `{{title}}`, `{{content}}`, `{{permalink}}`, `{{image}}`, `{{email}}`, `{{phone}}` and the datetime tags, the Source control builds a path: start at the current entry, the site or a relationship, then add steps that follow a relationship field or drop into a taxonomy term. Before this a source could follow one relationship and one taxonomy and no further, so something two relationships away was out of reach. Each step that can return several results carries its own limit: limit a taxonomy step to one and you get one term per referenced post, not one term overall. Leave it blank for all. Existing tags render as before, apart from the three source items under Fixed.
...
- **A limit of 0 now means no limit.** The per-slot limits on `{{join}}` and the `try_` tags, and any tag-wide Result Limit saved from before this release, all accepted 0, but rendered one result anyway. Entering 0 now returns every result the source has, there and on the new per-step limits. A hand-typed -1 reads the same way; 0 is what the plugin writes. **A tag saved with a limit of 0 or -1 starts listing every result after this update.** Set it to 1 to keep the previous output. A linked tag is only a link when a single result renders, so such a tag also stops being a link. Leaving a limit blank, or typing something that is not a number, means no limit stated: an older tag still bounds its list at one result, a source written as a path returns all of them.
...
- **The Tag Converter no longer discards a tag's on/off settings.** Four settings are written into a tag as a plain switch with no value: "Do not link" on the email and phone tags, "Open in new tab" on any linked tag, and the two date/time options for showing the current year and midnight. The converter silently dropped any such switch on a tag it rewrote, and has done since it shipped in 1.6. Until now the only ones it reached were the retired tag names carrying the pre-1.6 spelling of "Open in new tab", which it quietly turned off; this release converts far more tags, so all four were about to be exposed. All four now survive conversion. **If you converted a retired tag name on an earlier release, check whether it was meant to open in a new tab.** The setting cannot be recovered from the tag itself, though the converter saves a copy of each post before it changes anything.
...
- **The Tag Converter keeps the year and midnight display of a pre-1.6 date tag.** Those two settings were inverted when 1.6 renamed them, and the converter went on reading the old ones as though an untouched box meant the opposite of what it did. A pre-1.6 date or date-and-time tag whose author never ticked either box showed the current year and showed a time stored as midnight; converting it turned both off, so a date could quietly lose its year and a midnight time could vanish. Conversion now preserves what the tag showed. It also stops writing a midnight setting onto a date-only tag, or a year setting onto a time-only one, where neither does anything. Only tags predating the 1.6 rename are affected, and only those the converter has not already rewritten. **If you converted a pre-1.6 date or date-and-time tag on an earlier release, check how it displays the year and midnight times.** A converted tag no longer records what it was set to, so this one cannot be put right automatically, though the converter saves a copy of each post before it changes anything.
