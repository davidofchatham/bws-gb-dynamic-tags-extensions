...
- **For plugin developers: two routes to offering a source as a source-path start.** A source class returns `true` from `is_selectable_root()`, or a plugin declares one from the `bws_dynamic_tags_chain_roots` filter with a label, a context type and a function that finds the entity - no class needed. Both land in the same registry the renderer already reads, so an offered start is always one that resolves. Offering is separate from resolving throughout: a tag naming your source resolves whether or not you currently offer it, so removing the offer never blanks saved content. See [`docs/plugin-integration.md`](docs/plugin-integration.md) §1a.
