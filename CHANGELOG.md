...
- **Result Limit stays visible once a source follows a step.** It used to disappear the moment a source was written as a path rather than picked from the old dropdown, because it was shown by matching the two source values that were once the only ones returning several results. It is now shown whenever the source can return several results, however that source is written. The Result Separator follows the same rule.
- **The Tag Converter rewrites saved sources into paths.** Running it over your content converts a tag's Source, Relationship Field Key and taxonomy setting into a single source path, and writes in the result limit the old spelling implied so output does not change. Opening a tag in the editor does the same thing, and the two cover different content: the converter reaches drafts and posts nobody has opened, while opening a tag reaches block widgets and templates the converter does not scan. A tag you never touch keeps working as it is.
- **A source you convert to a path keeps the result limit it had.** A source written the old way returns one result unless you say otherwise; a source written as a path returns all of them. So when you change a tag's source into a path, the editor writes the old limit of 1 into Result Limit and leaves it there for you to see and clear, rather than letting the tag quietly start listing everything. Clearing it is how you ask for the whole list. Tags you never touch are unaffected: an untouched tag is still written the old way, and still returns one result.
...
- **The image and date/time return-type controls are labelled the way the reference describes.** An image tag's return type was labelled "Return type:" on `{{image}}` and "Return image as:" on `{{term_image}}` and `{{try_image}}`, for one control doing one job; all three now read "Return type". The date/time tags' equivalent read "Show:" and now reads "Return As". Labels only, and the size dropdown under an image's return type is no longer pressed up against it.
