...
- **Tag option controls are grouped in the editor.** Controls that describe one decision now sit together in a single bordered panel: the source and everything that shapes it (Result Limit and Result Separator), the field selection, how the value is rendered (the datetime formatting options, an image's return type and size, `{{join}}`'s assembly mode and its separator or template), and the link options. Multi-slot tags group their controls this way already, so a field on a `{{join}}` slot and the same field on `{{text}}` now read alike. A group that has only one control showing does not draw a panel, so a tag with no link set does not grow an empty box. Nothing about the saved tag or the output changes.
