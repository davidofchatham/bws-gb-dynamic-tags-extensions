...
- **Tag option controls are grouped in the editor.** Controls that describe one decision now sit together in a single bordered panel: the source and everything that shapes it (Result Separator, and Result Limit where a tag still has one), the field selection, how the value is rendered (the datetime formatting options, an image's return type and size, `{{join}}`'s assembly mode and its separator or template), and the link options. Multi-slot tags group their controls this way already, so a field on a `{{join}}` slot and the same field on `{{text}}` now read alike. A group that has only one control showing does not draw a panel, so a tag with no link set does not grow an empty box. Nothing about the saved tag or the output changes.
...
- **The tag-wide Result Limit control is gone from the tags that build a source path.** `{{text}}`, `{{title}}`, `{{email}}`, `{{phone}}`, the datetime tags and the `try_` family no longer show it. A limit is stated where the source is stated: a path states its source as steps, so each step that can return several results carries its own limit, and a tag-wide number on top of that is either the same setting written twice (with one step) or a slice at a position you cannot see (with two). Tags that pick their source from a dropdown rather than building a path are where the control belongs, since for them the source is a single step. Nothing renders differently. A limit already saved in a tag is still read and still bounds the list, whether the Tag Converter has reached that tag or not, so a tag in a widget or a template nobody opens keeps working, and a tag string you write by hand still means what it says. Where a tag whose source is already a path carries one, opening it moves the number onto the step it applies to, so the cap you are getting is the cap you can see and clear. Output does not change.
