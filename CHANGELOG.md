...
- **A limit of 0 now means no limit.** Limits on `{{join}}`, the `try_` tags and any saved tag-wide Result Limit all accepted 0 and rendered one result anyway. A tag saved with 0 or -1 starts listing every result after this update; set it to 1 to keep the previous output. Note that on `{{text}}`, `{{title}}` and the date/time tags one link wraps the whole result, so a tag that starts listing loses its link; the email and phone tags are unaffected, since each address or number is its own link. (Changed)
- **A tag's source can now follow more than one step.** Start at the current entry, the site or a relationship, then add steps that follow a relationship field or drop into a taxonomy term, each with its own Limit results. Something two relationships away used to be out of reach; that is also what made 25 retired tag names convertible again. (Added)
- **Multi-slot tags configure one slot at a time, with buttons to add and remove them.** `{{join}}` and the nine `try_` tags used to reveal their slots one at a time, and clearing an early slot could hide the later ones. Each slot is now a block of controls you add and remove directly, and removing a middle slot closes the gap. Existing tags are converted for you, and output is unchanged. (Changed)
- **A source that names a relationship but no field renders nothing, instead of quietly reading the current entry.** Such a tag showed a real value from the wrong place, so nothing looked broken. If a spot on your site goes blank after this update, open that tag and finish its source: it was reading something it was never pointed at. (Fixed)

### Added

- **A tag's source path can now follow more than one step.** On the base tags, the Source control builds a path: start at the current entry, the site or a relationship, then add steps that follow a relationship field or drop into a taxonomy term. Before this a source could follow one relationship and one taxonomy and no further, so something two relationships away was out of reach. Each step that can return several results carries its own Limit results: limit a taxonomy step to one and you get one term per referenced post, not one term overall. Leave it blank for all. Once an earlier step returns several results, a step's limit applies to each of them in turn, and the control says so. Existing tags render as before, apart from the three source items under Fixed.
...
- **25 retired tag names are now convertible.** The `second_related_post_*` and `post_term_related_post_*` families were retired in 1.6, and the Tag Converter has had no conversion for them since, because nothing could follow two relationships. A source path can. **This is the one change here you may see on a page**: these tags have rendered nothing since 1.14, so a blank spot can start showing content again after you run the converter. A tag whose second relationship field was never filled in is left alone rather than half-converted.
- **Another plugin can add its own starting point to the Source control.** A plugin that works out its own entry from the page being viewed can offer it as a place a source path starts. It then appears in the Source list on every base tag and in every slot of `{{join}}` and the first-available tags, named by that plugin, next to Current and Site. Steps, per-step limits, field pickers and previews all work from there as usual. Nothing appears until a plugin you have installed opts in.
...
- **The Tag Converter rewrites saved sources into paths.** A tag's Source, Relationship Field Key and taxonomy setting become a single source path. The old spelling returned one result unless you said otherwise, while a path returns all of them, so each step's limit is set to 1 and left there for you to see and clear. A Result Limit you set yourself moves onto the last step instead. Opening a tag in the editor does the same, and the two cover different content: the converter reaches drafts and posts nobody has opened, while opening a tag reaches block widgets and templates the converter does not scan. A tag you never touch keeps working as it is.
...
- **The tag-wide Result Limit control is gone.** `{{text}}`, `{{title}}`, `{{email}}`, `{{phone}}`, the datetime tags and the `try_` family no longer show it, and no other tag ever had one. A limit is stated where the source is stated: each step that can return several results carries its own. A limit already saved in a tag is still read and still bounds the list, converted or not, so a tag in a widget and a tag string you wrote by hand both keep working. Opening a tag whose source is already a path moves the number onto the step it applies to. Output does not change.

### Fixed

- **`{{content}}` reading a related post now shows that post's own values.** Pointing the content tag at a relationship pulled in the related post's layout, but any tag inside that layout read the page you were viewing instead of the post it belonged to. The right structure came out carrying the wrong values, which looks like working output rather than a fault. Excerpts had it too, including the "read more" link, which pointed back at the page you were on. Both now resolve against the post the content came from. This affects the tag only when its source is a relationship or a path that leads elsewhere; reading the current entry is unchanged.
- **A source that names a relationship but no field renders nothing, instead of quietly reading the current entry.** Setting a tag's source to a related post and leaving Relationship Field Key empty used to fall back to the entry the tag sits on, so the tag showed a real value from the wrong place and nothing looked broken. The same went for a step with no field or taxonomy chosen yet. Such a tag now renders nothing, which is what the editor has been telling you all along: an unfinished step displays "will be skipped unless a field is set". On `{{join}}` and the first-available tags the unfinished slot is skipped, so a later attempt gets its turn. **If a spot on your site goes blank after this update, open that tag and finish its source: it was reading something it was never pointed at.** A step that inherits its field from an earlier slot is unaffected.
...
- **A slot set to "Same as Previous Source" inherits the whole of that source, and inherits nothing when there is nothing to reuse.** Two faults, both ending in a silent read of the entry the tag sits on.
  - **A taxonomy term step was not inherited.** On `{{join}}` a field you expected from the term came back blank, or worse, came back with the page's own value and looked right. On the first-available tags the same attempt could succeed against the wrong entry and stop the ones after it from being tried. A slot that states its own source or its own taxonomy still wins. **If a multi-slot tag of yours starts at a taxonomy term, check the slots after the first: they may have been reading the current entry and will now read the term.**
  - **With every earlier slot skipped, there was no previous source at all.** If none had a field chosen yet, the slot fell back to the current entry rather than to nothing, so a `{{join}}` whose only visible source said the site read the page instead. Slot one is unchanged: a first slot with no source set still means the current entry.
...
- **The Tag Converter no longer loses a tag's on/off settings.** Four settings are written into a tag as a plain switch with no value: "Do not link" on the email and phone tags, "Open in new tab" on any linked tag, and the two date/time options for showing the current year and midnight. Two separate faults could leave a converted tag with one of them turned off. Both are fixed. In both cases the setting cannot be recovered from the tag itself, though the converter saves a copy of each post before it changes anything.
  - **Any such switch was deleted outright by every conversion,** and had been since the converter shipped in 1.6. Until now the only one it reached was the pre-1.6 spelling of "Open in new tab" on a retired tag name, which it quietly turned off; this release converts far more tags, so all four were about to be exposed. All four now survive conversion. **If you converted a retired tag name on an earlier release, check whether it was meant to open in a new tab.**
  - **On a pre-1.6 date tag, the year and midnight settings were inverted on top of that.** 1.6 renamed both, and the converter went on reading the old ones as though an untouched box meant the opposite of what it did. A pre-1.6 date or date-and-time tag whose author never ticked either box showed the current year and showed a time stored as midnight; converting it turned both off, so a date could quietly lose its year and a midnight time could vanish. Conversion now preserves what the tag showed, and stops writing a midnight setting onto a date-only tag, or a year setting onto a time-only one, where neither does anything. Only tags predating the 1.6 rename are affected, and only those the converter has not already rewritten. **If you converted a pre-1.6 date or date-and-time tag on an earlier release, check how it displays the year and midnight times.**
- **The Tag Converter's repair of the pre-1.6 relationship key is fixed on three counts.** That repair rewrites the old spelling (`rel:`) to the current one (`ref:`); only content old enough to predate the 1.6 rename is affected.
  ...
  - **It no longer re-points a working tag.** Since 1.6, a tag carrying both spellings had the old one win: the repair overwrote the live `ref:` value with the `rel:` value beside it, and the tag silently started reading a different post. The tag's source now decides which spelling was actually in use, and the other is dropped. Only tags holding both keys at once were affected, which the editor cannot produce; a hand-edited tag or one left by a partial converter run can.
