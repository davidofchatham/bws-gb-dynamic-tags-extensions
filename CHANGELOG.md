...
- **The limit control is no longer offered on the Content, Permalink and Image tags**, including their try_ variants. These tags show one result, so there is nothing for the control to bound, and every limit saved in such a tag is ignored at render. A previously saved limit stays in the tag untouched. In its place, a note now appears under the source controls when the source step can return more than one item, stating that only the first one is read.
