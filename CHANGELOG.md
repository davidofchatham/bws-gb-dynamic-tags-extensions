...
- **A tag's source can now follow more than one step.** The Source control on `{{text}}`, `{{title}}`, `{{content}}`, `{{permalink}}`, `{{image}}`, `{{email}}`, `{{phone}}` and the datetime tags builds a path: start at the current entry, the site, or a relationship, then add a step to follow a relationship field or drop into a taxonomy term, and add another after that. Before this you could follow one relationship and one taxonomy and no further, so something two relationships away (the office of the staff member an event references) could not be asked for at all. You add steps one at a time and see what each one does before committing to the next. Each step that can return several results carries its own optional limit, meaning at most that many from each incoming result, so limiting a taxonomy step to one gives you one term from each referenced post rather than one term overall. Leave that box blank for all. Existing tags keep rendering exactly as they do now, with nothing to run and nothing to review, apart from the three source fixes listed under Fixed.
- **25 old tag names the Tag Converter could never fix are now convertible.** The `second_related_post_*` and `post_term_related_post_*` families were retired in 1.6, and the converter has listed them ever since as having no conversion available, because nothing in the plugin could follow two relationships or a taxonomy term and then a relationship. A source path states both, so those tags now convert. **This is the one change here you may see on a page**: these tags have rendered nothing since 1.14, so a spot that has been blank can start showing content again after you run the converter. Everything else in this release renders exactly as it did, apart from the three source fixes listed under Fixed. A tag whose second relationship field was never filled in is left alone rather than half-converted, since there is no way to guess what it was meant to point at.
- **A relationship field stored on an options page can now be followed.** A repeater held there was already readable; a relationship was not, for no reason that survived inspection. A globally configured "featured partner" now works the way a per-post one does.
- **Another plugin can add its own starting point to the Source control.** A plugin that works out its own entry from the page being viewed can now offer that as a place a source path starts, so it appears in the Source list on every base tag and in every slot of `{{join}}` and the first-available tags, named by that plugin, next to Current and Site. Choosing it builds an ordinary source path from there: steps, per-step limits, field pickers and previews all work as they do anywhere else. Before this the only way a plugin could surface its own entry was to add a whole parallel family of tags, which duplicated the base tags and then fell behind them. Nothing appears in your Source list until a plugin you have installed opts in, and a plugin can also drop the offer again without breaking anything: tags you already saved go on rendering either way. Where such a tag cannot be worked out in the editor, its preview names the source in words rather than showing you a code.
- **For plugin developers: two routes to offering a source as a source-path start.** A source class returns `true` from `is_selectable_root()`, or a plugin declares one from the `bws_dynamic_tags_chain_roots` filter with a label, a context type and a function that finds the entity - no class needed. Both land in the same registry the renderer already reads, so an offered start is always one that resolves. Offering is separate from resolving throughout: a tag naming your source resolves whether or not you currently offer it, so removing the offer never blanks saved content. One upgrade note: `is_selectable_root()` is declared on `SourceInterface`, so a source class that implements the interface **directly** must add it; extending `AbstractSource`, which is what the guide recommends, inherits the default and needs no change. See [`docs/plugin-integration.md`](docs/plugin-integration.md) §1a.

### Changed

- **Multi-slot tags now configure one slot at a time, with buttons to add and remove them.** `{{join}}` and the nine `try_` tags used to reveal their slots one at a time: slot 3's controls appeared only once slot 2 held a value, so building a chain meant filling slots in order, and clearing an early slot could hide the later ones. Each slot is now a single block of controls you add and remove directly. Removing a middle slot closes the gap, and a later slot set to inherit from the one you removed keeps the value it was reading rather than silently re-pointing at a different source. Each slot's whole configuration is saved under one option key per slot, so a slot's source and field stay together in the tag string instead of scattering across six separate keys. Output is unchanged apart from the source fixes listed under Fixed: a slot whose source is written as a path returns every result, so converting one sets that slot's step limit to the single result the old spelling implied, exactly as converting a tag's own source does. Existing tags are converted for you, in two places: when you open one in the editor, and when you run the Tag Converter over saved content. The two paths cover different things, so both still matter: the converter reaches drafts and posts nobody has opened, and opening a tag reaches block widgets and templates the converter does not scan. Each slot's key is a letter (`A`, `B`, `C`, and so on), which is also what the slot's panel is labelled, so a slot's label and its place in the tag string read the same. Each tag names its slots in its own terms: `{{join}}` adds a field (`Field A`), a `try_` tag adds an attempt (`Attempt A`). 1.16.0's note that the return type or format leads the saved string still holds: the slots follow it.
- **A tag whose source is a path no longer shows the two old source fields as well.** Relationship Field Key and "Get from taxonomy term?" are now part of the source path itself, shown as steps, so the separate controls are gone from the tags that build a path: `{{text}}`, `{{title}}`, `{{content}}`, `{{permalink}}`, `{{image}}`, `{{email}}`, `{{phone}}` and the datetime tags. A tag saved the old way still opens with its relationship and taxonomy showing as steps, and still saves as before if you do not touch it. It renders as before too, unless it is one of the shapes under Fixed: a source naming a relationship with no field, or a multi-slot tag whose later slots reuse a source that starts at a taxonomy term. Tags that do not build a path keep both controls unchanged: the `term_` family, `{{table}}` and `{{call}}`.

- **Tag option controls are grouped in the editor.** Controls that describe one decision now sit together in a single bordered panel: the source and everything that shapes it (Result Separator, and the source path with the limit on each of its steps), the field selection, how the value is rendered (the datetime formatting options, an image's return type and size, `{{join}}`'s assembly mode and its separator or template), and the link options. Multi-slot tags group their controls this way already, so a field on a `{{join}}` slot and the same field on `{{text}}` now read alike. A group that has only one control showing does not draw a panel, so a tag with no link set does not grow an empty box. Nothing about the saved tag or the output changes.

- **`{{join}}`'s format tokens are now letters, matching the slot labels.** In template mode you write `%A`, `%B`, and so on, where 1.15.0 used `%1`, `%2`. The letter is the same one the slot's own panel is labelled with, so a format string and the slots it refers to read together. The numbered tokens still work and will keep working, so a format string you already wrote needs no change and a tag pasted from older notes still renders. One thing to know if you used a literal percent sign: a `%` directly before a capital A through J is now read as a slot token, so text like `10%APR` would change meaning. Run the Tag Converter and it rewrites those to `%%APR`, which renders as typed. If you type a new format string by hand, use `%%` before a capital letter you mean literally.
- **A limit of 0 now means no limit.** Every limit accepted 0 and then quietly rendered one result anyway: the limit on each step of a source path, the per-slot limits on `{{join}}` and the `try_` tags, and any tag-wide Result Limit still saved in a tag from before this release. Entering 0 in any of those returns every result the source has, so you no longer have to guess a high number. A hand-typed -1 is read the same way, since that is the convention GenerateBlocks uses elsewhere; 0 is what the plugin writes. A value that is not a number falls back to one result rather than to no limit, so a typo cannot open the floodgates. **If you have a tag saved with a limit of 0 or -1, it starts listing every result after this update.** Set it to 1 to keep the previous output; on a tag that builds a source path, that number now lives on the step, since the tag-wide field is gone (below). Note that a linked tag wraps its output in a link only when a single result renders, so such a tag also stops being a link once it shows several results. A saved tag with no limit at all is unchanged: it still returns one result, as does any value of 1 or more.
- **Result Separator stays visible once a source follows a path.** It used to disappear the moment a source was written as a path rather than picked from the old dropdown, because it was shown by matching the two source values that were once the only ones returning several results. It is now shown whenever the source can return several results, however that source is written. Result Limit went the other way and is gone from these tags entirely (below): a limit is stated where the source is stated, and a path states its source as steps.
- **The Tag Converter rewrites saved sources into paths.** Running it over your content converts a tag's Source, Relationship Field Key and taxonomy setting into a single source path, and sets each step's limit to the one result the old spelling implied so output does not change. Opening a tag in the editor does the same thing, and the two cover different content: the converter reaches drafts and posts nobody has opened, while opening a tag reaches block widgets and templates the converter does not scan. A tag you never touch keeps working as it is.
- **A source you convert to a path keeps the result limit it had.** A source written the old way returns one result unless you say otherwise; a source written as a path returns all of them. So when you change a tag's source into a path, the editor sets the limit on each step of that path to 1 and leaves it there for you to see and clear, rather than letting the tag quietly start listing everything. The number appears in the step it applies to, next to the relationship or taxonomy it bounds. Clearing it is how you ask for the whole list. If the tag already had a Result Limit you set yourself, that number moves onto the last step of the path rather than being left behind, so the limit is stated in one place. Tags you never touch are unaffected: an untouched tag is still written the old way, and still returns one result.
- **On the first-available tags, Result Limit belongs to each attempt now.** `{{try_text}}` and its siblings had a single tag-wide Result Limit, and it never bounded the tag as a whole: it was the number each attempt used for its own list, so a tag with three attempts applied it three times. Once an attempt's source is written as a path there is nothing to say which step of that path such a number applies to, and no way to aim it, so it stops being a tag-wide setting. Converting a tag moves the number you set onto each attempt that lists something, on the step it applies to, and removes the tag-wide field. Output is unchanged, including for an attempt set to inherit the previous one's source: inheriting a source now inherits its limit as well, which is what that setting always did for you. One thing about inheriting attempts did change, and it is a fix rather than a side effect: an attempt inheriting a source that starts at a taxonomy term now follows the term, where before it read the current entry. See Fixed. An attempt that returns a single value gets no limit, because it never had a list to bound. `{{join}}` is unaffected, since its Result Limit has always belonged to the individual field rather than to the tag.
- **For plugin developers: the registration helper `bws_strip_default_select_values()` is now `bws_prepare_registration_options()`.** There is no alias, so an add-on calling the old name needs updating. The name changed because the function did: as well as stripping default select values it now stamps each option's visual group and drops the flat source controls a source path has taken over. Nothing else about it changed, including what it takes and what it returns. See [`docs/plugin-integration.md`](docs/plugin-integration.md). No effect on tags or output.
...
- **Relationship traversal now has one setting, not two spellings.** Four built-in sources kept resolving relationships themselves, reading the field name from an option called `rel` (or, further back, `key`) that nothing else in the plugin has read since the traversal rework in 1.14.0. Everything current reads Relationship Field Key (`ref`). Those four no longer resolve anything, so `ref` is the only spelling that means anything. A saved tag naming `related_post` as its source is rewritten to use `ref` when you run the Tag Converter, which keeps it reading what it read before. This was only reachable by hand-writing a tag or by content old enough to predate the conversion, so tags built in the editor were never affected. If you did hand-write one with `rel:`, check it: the converter can only fix what it can find.

### Removed

- **The tag-wide Result Limit control is gone.** `{{text}}`, `{{title}}`, `{{email}}`, `{{phone}}`, the datetime tags and the `try_` family no longer show it, and those were the only tags that ever had it, so no tag has one now. A limit is stated where the source is stated: a path states its source as steps, so each step that can return several results carries its own limit, and a tag-wide number on top of that is either the same setting written twice (with one step) or a slice at a position you cannot see (with two). Nothing renders differently. A limit already saved in a tag is still read and still bounds the list, whether the Tag Converter has reached that tag or not, so a tag in a widget or a template nobody opens keeps working, and a tag string you write by hand still means what it says. Where a tag whose source is already a path carries one, opening it moves the number onto the step it applies to, so the cap you are getting is the cap you can see and clear. Output does not change.

### Fixed

- **The Tag Converter no longer discards a tag's on/off settings.** Four settings are written into a tag as a plain switch with no value: "Do not link" on the email and phone tags, "Open in new tab" on any linked tag, and the two date/time options for showing the current year and midnight. The converter read every other setting and silently dropped these, so running it removed them. The visible effect was on "Do not link": an email or phone tag set not to link came back linked, with nothing left in the tag to say otherwise. The other three reverted to their defaults the same way. All four now survive conversion. **If you have run the converter on a previous release, check any email or phone tag you had set not to link, and any tag set to open in a new tab.** The setting cannot be recovered from the tag itself, though the converter saves a copy of each post before it changes anything, so an older revision may still show what was there. This has been the behaviour since the converter shipped in 1.6.

- **An image tag saves its return type again.** Since 1.16.0, a newly inserted image tag wrote no return type into the tag string at all, so `{{image use:featured}}` did not say it returned a URL at full size. It rendered correctly, because a missing return type has always meant URL at full size, but the point of writing it out is that you can read a tag and see what it does, and change `as:url` to `as:alt` in one edit without opening the panel. New image tags write it again. Tags saved under 1.16.0 keep rendering as they always did; open one and set the return type if you want it written out.

- **A slot set to "Same as Previous Source" now follows the taxonomy term the previous slot did.** A source that drops into a taxonomy term left the following slot with nothing to inherit, so a slot set to reuse it silently read the entry the tag sits on instead. On `{{join}}` that meant a field you expected from the term came back blank, or worse, came back with the page's own value and looked right. On the first-available tags the same attempt could succeed against the wrong entry and stop the ones after it from being tried. Inheriting a source now inherits the whole of it, including the term step, in both kinds of multi-slot tag. A slot that states its own source is unaffected, and a slot that states its own taxonomy still wins over the inherited one. Result Limit is unchanged and still belongs to each `{{join}}` field individually, since a limit is a setting on a source rather than part of what the source is. **If a multi-slot tag of yours starts at a taxonomy term, check the slots after the first: they may have been reading the current entry and will now read the term.** Tags whose first slot does not use a taxonomy term are unaffected.

- **A slot set to "Same as Previous Source" renders nothing when there is no previous source to reuse.** If every earlier slot was skipped, for instance because none had a field chosen yet, the slot fell back to the current entry rather than to nothing, so a `{{join}}` whose only visible source said the site read the page instead. Slot one is unchanged: a first slot with no source set still means the current entry, which is what it has always meant.

- **A source that names a relationship but no field renders nothing, instead of quietly reading the current entry.** Setting a tag's source to a related post and leaving Relationship Field Key empty used to fall back to reading the entry the tag sits on, so the tag showed a real value from the wrong place and nothing looked broken. The same went for a step in a source path with no field or taxonomy chosen yet. Such a tag now renders nothing, which is what the editor has been telling you all along: an unfinished step already displays "will be skipped unless a field is set", and only the multi-slot tags were doing it. On `{{join}}` and the first-available tags the unfinished slot is skipped, so a later attempt gets its turn rather than the first one succeeding against the wrong entry. **If a spot on your site goes blank after this update, open that tag and finish its source: it was reading something it was never pointed at.** A step that inherits its field from an earlier slot is unaffected, since that step does have a field.
- **The first-available and term-based tags order their controls like every other tag now.** On the nine `try_` tags, the formatting options rendered above the attempts rather than below them, Fallback Text sat above the link options, and Result Limit and Result Separator were pushed to the very bottom of the panel, below everything else, even though they describe the source at the top. On the `term_` tags, Fallback Text likewise sat above the link options. All of them now run source, then field, then formatting, then link, then fallback: the order 1.16.0 described and the order the other tags already used. Nothing about the saved tag or the output changes.
...
- **That repair no longer re-points a working tag.** Since 1.6, a tag carrying both the old and the new relationship key spellings had the old one win: the repair overwrote the live `ref:` value with the `rel:` value beside it, and the tag silently started reading a different post. The tag's source now decides which spelling was actually in use, and the other is dropped. Only tags holding both keys at once were affected, which the editor cannot produce; a hand-edited tag or one left by a partial converter run can.
...
- **`{{content}}` reading a related post now shows that post's own values.** Pointing the content tag at a relationship pulled in the related post's layout, but any tag inside that layout read the page you were viewing instead of the post it belonged to. A staff page whose content shows a name field rendered on another page came out with that page's name in it: the right structure carrying the wrong values, which looks like working output rather than a fault. Excerpts had it too, including the "read more" link, which pointed back at the page you were on rather than at the post being summarised. Both now resolve against the post the content came from. This affects the tag only when its source is a relationship or a source path that leads elsewhere; reading the current entry is unchanged.
