...
- **Optional leading-country-code strip** (global **Phone → "Strip a leading country code matching the default"**, default OFF) guards stored-with-CC-but-no-`+` numbers (e.g. US `1-800-555-1212` with country code `1`) from a doubled prefix. Matches the configured global country code only.
