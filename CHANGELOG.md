...
- **A Result Limit of 0 now means no limit.** Every limit accepted 0 and then quietly rendered one result anyway: the tag-wide Result Limit saved on `{{text}}`, `{{title}}`, `{{datetime_single}}`, `{{datetime_range}}`, `{{email}}` and `{{phone}}`, the per-slot limits on `{{join}}` and the `try_` tags, and the per-step limit a source path now carries. Entering 0 now returns every result the source has, so you no longer have to guess a high number. A hand-typed -1 is read the same way, since that is the convention GenerateBlocks uses elsewhere; 0 is what the plugin writes. Leaving Result Limit blank still returns one result, and so does any value of 1 or more, both unchanged. A value that is not a number falls back to one result rather than to no limit, so a typo cannot open the floodgates. **If you have a tag saved with a limit of 0 or -1, it starts listing every result after this update.** Set it to 1 to keep the previous output. Note that a linked tag wraps its output in a link only when a single result renders, so such a tag also stops being a link once it shows several results.
