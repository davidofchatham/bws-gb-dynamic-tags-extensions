...
- **A tag's source can now follow more than one step.** Start at the current entry, the site or a relationship, then add steps that follow a relationship field or drop into a taxonomy term, each with its own limit. Something two relationships away used to be out of reach; that is also what made 25 retired tag names convertible again. (Added)
