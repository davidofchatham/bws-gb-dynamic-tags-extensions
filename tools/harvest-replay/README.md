...
- **The migration replay — the WIRE changed**, and it is real wire nobody designed for. Baseline
  ...
  **`--removed=` is not optional in practice.** The GB Pro pattern-cache repair REMOVES stale shadow wire, so the C-side census legitimately holds fewer rows and every removed string lands as a pair present on only one side — 212 of them beside 13,445 identical on one measured run, every one a hard failure. The artifact `run-converter.php` writes says which strings the repair cleared, so those report as repairs and every other one-sided pair stays the failure it was. It is evidence the run produced, not an exception list: an artifact naming nothing forgives nothing.

  **Measured end to end on the Site H clone 2026-09-15**, the full four-step run against a genuine pre-1.17.0 snapshot (A on the released 1.19.0 build, converter on 1.20.0, C on 1.20.0; corpus 345 census rows over 105 URLs): 6 distinct strings cleared by the repair, **210 one-sided pairs, every one of them reported as a repair rather than a failure**, beside 13,805 identical and 120 volatile excluded. That is the shape the flag exists for, produced by a run rather than by synthetic artifacts. The same run's residue triages cleanly and is worth knowing as the reading practice: 40 CHANGED, all of them `{{image …|fallback:…}}` on archive / search / 404 / date contexts, which is the `{{image}}` fallback fix released in 1.19.1 — a BUILD-axis change, visible because the clone's live copy sat two patch releases behind dev, and not a migration finding. And 315 A-side renders paired on the OLD wire, which is exactly the 3 census strings `run-converter.php` reported as unreachable (options/meta only) times the 105 URLs. **A migration replay moves both axes by construction**, so a CHANGED bucket is attributed to one of them before it is read as a regression.
