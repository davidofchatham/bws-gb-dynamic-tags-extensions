...
| FR5.5 | Any base tag's Source control | No row for the four retired traversal-substitute sources — a registry that keeps its dead must not leak it into an authoring surface. **Post and Term DO appear** (1.20.0, FW-39): each is now a pinning root, not an internal-only key; selecting either mounts the entity-picker control (D11/D15) rather than a plain enum row. The dedicated pinned-root eyeball (picker browsing, filtering, the resolved-label caption, the "nothing pinned" warning) lives on `/matrix-pinned-roots/` — see `fold-test-matrix.md` §F20 |
