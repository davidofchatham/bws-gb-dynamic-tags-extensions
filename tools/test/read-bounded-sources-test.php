<?php
/**
 * Standalone unit harness for bws_read_bounded_sources() — the bounded source READER
 ...
 *
 * DORMANT PREDICATE PATH (bws_value_is_populated, FW-88 — the function itself is
 * live as the default path's emptiness test, but nothing shipped passes it AS a
 * predicate; that role is pinned here so it cannot rot while dormant):
 ...
 * the render.
 *
 * Run:  php tools/test/read-bounded-sources-test.php   (exit 0 = pass, 1 = fail)
 *
...
$calls = array();
eq( 'D1 empty source list → empty set', array(), bws_read_bounded_sources( array(), reader( array(), $calls ), 1 ) );
...
eq(
	...
	array(),
	bws_read_bounded_sources( array( 'a', 'b', 'c' ), reader( array( 'a' => '', 'b' => '', 'c' => 'v3' ), $calls ), 1 )
);
...
eq(
	...
	array( 'v1' ),
	bws_read_bounded_sources( array( 'a', 'b' ), reader( array( 'a' => 'v1', 'b' => 'v2' ), $calls ), 1 )
);
...
eq(
	...
	array( 'v1' ),
	bws_read_bounded_sources( array( 'a', 'x', 'b' ), reader( array( 'a' => 'v1', 'x' => '', 'b' => 'v2' ), $calls ), 2 )
);
...
eq(
	...
	array( 'v1', 'v2', 'v3' ),
	bws_read_bounded_sources( array( 'a', 'x', 'b', 'c' ), reader( array( 'a' => 'v1', 'x' => '', 'b' => 'v2', 'c' => 'v3' ), $calls ), 0 )
);
...
eq(
	...
	array( '0' ),
	bws_read_bounded_sources( array( 'f', 'n', 'z' ), reader( array( 'f' => false, 'n' => null, 'z' => '0' ), $calls ), 0 )
);
...
eq(
	...
	array( 'p' ),
	bws_read_bounded_sources( array( 'list', 'one' ), $multi, 1, $pred )
);
eq( 'P3 predicate: false/null fail, string "0" passes', true, bws_value_is_populated( '0' ) && ! bws_value_is_populated( false ) && ! bws_value_is_populated( null ) && ! bws_value_is_populated( '' ) );
