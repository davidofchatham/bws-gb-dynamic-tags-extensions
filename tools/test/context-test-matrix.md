...
## C-TERM / CT rows — a term SOURCE versus a term CONTEXT

**What this section used to be, and what outlived it.** An argless `{{term_*}}` tag resolved through `TaxonomyTerm::resolve_id()`, which handed back `get_queried_object_id()` without checking what kind of thing WP had queried; post, term and user ids share one number space, so wherever the queried object's id collided with a real term the tag rendered that term's data as though it were the answer. 1.20.0 gated that arm on the queried object's type and these rows were the measurement. 1.21.0 removed the arm along with the family that was its only caller (FW-129), so the guard is not a thing any row can measure any more — `TaxonomyTerm::resolve_id()`'s PHPDoc states what the resolution does now. What the rows rode on and what they still pin is the distinction underneath: **a term SOURCE and a term CONTEXT are different things, and only the second moves with the page.**

**Rewritten against base spellings 2026-09-16 (FW-129), and the section shrank, which is the honest outcome rather than a trim.** The old rows came in pairs because the guard's correct answer was "empty" on six of seven contexts, and a set of all-empty rows cannot tell a working guard from a tag that stopped resolving anywhere — so every ambient row needed a named-term twin that had to keep rendering. In base spellings the ambient arm is never empty: `{{text use:title}}` renders whatever the context is about, which is C1–C7 above. The non-vacuity problem the pairs existed for does not arise, and with it gone most of the old rows turned out to be spellings already carried by visible rows elsewhere, so they point rather than duplicate.

| # | Context / page | URL | Ambient — `{{text use:title}}` | Stated — `{{text src:term,<support>\|use:title}}` | Surface (visible row) |
|---|---|---|---|---|---|
| C-TERM1.1 | Term archive (the arm the contrast needs) | `/department/sales/` | `Sales` — the queried term, and the only context where the two columns read the same KIND of thing | `Support` — a different term on the one page that is about a term | C-element, `ctx-term`: C-X1 / C-CONV11 |
| C-TERM1.2 | Author archive | `/author/fixture-author/` | the context's own subject (C3) | `Support` | C-element, `ctx-author`: C-X1 / C-CONV11 |
| C-TERM1.3 | Post type archive | `/staff/` | the context's own subject (C2) | `Support` | C-element, `ctx-pta-staff` |
| C-TERM1.4 | Date archive | `/2026/07/` | the context's own subject (C1) | `Support` | C-element, `ctx-date-202607` |
| C-TERM1.5 | Search | `/?s=searchpin` | the context's own subject (C4) | `Support` | C-element, `ctx-search` |
| C-TERM1.6 | 404 | `/no-such-page-xyz/` | the context's own subject (C5) | `Support` | C-element, `ctx-404` |
| C-TERM1.7 | Latest-posts home | `/` | the context's own subject (C6) | `Support` | C-element, `ctx-home-latest` |
| CT-A | Singular page | `/matrix-pinned-roots/` | `Matrix: Pinned Entity Roots` — this page, never a term | — | **F20.2**, `page-matrix-pinned-roots` |
| CT-B | Singular page | `/matrix-pinned-roots/` | — | `Sales` (this page pins Sales, not Support) — unmoved by a singular context | **F20.1**, `page-matrix-pinned-roots` |
| CT-C | Singular page | `/matrix-post-meta/` | — | `(987) 333-4444` — `{{text src:terms,department,limit(1)\|key:phone}}`, the first term OF this post rather than a term the page is about | **CT-C**, `page-matrix-post-meta` |
| QL1.6 | Term query loop | `/matrix-loops/` | the loop's term name | — | **QL1.6**, `page-matrix-loops`, inside QL1's loop |

**The stated column is one tag, and it is deliberately not the term any of these pages is about.** `<support>` is resolved at build time (`bws_fixture_seeded_term_id`), and `/department/sales/` is the term archive, so on the one context where an ambient read has a term to find, the two columns disagree. A row stated at Sales would pass on the term archive whether the source resolved or not.

**Three rows moved off `/matrix-post-meta/` rather than being rewritten there.** CT-A and CT-B were an ambient read and a named-term read on a singular page, and `/matrix-pinned-roots/` already carries both in base spellings as F20.2 and F20.1 — a third copy would pin nothing new. CT-C stayed because it is the one that needs the page's own fixture state: a `terms` step reads the terms of the current post, and the pinned-roots page carries none. It is also the tier the old ambient read starved — `tax` meant "the first term of the current post in this taxonomy" and the unchecked ambient read answered before that tier could run, so the option described a behavior it could not deliver.

**QL1.5 is gone and QL1.6 inherits its whole job.** QL1.5 was the `term_` family reading the loop's term through `TaxonomyTerm::resolve_id()`; its base spelling was already sitting under it as QL1.6, through `bws_resolve_base_source()`'s loop-item classification. A query loop is the one context `render-tag` cannot fake at any flag combination — the loop hands its row's term down through GB's `generateblocks_dynamic_tag_id` filter, not through the queried object — so this page is the only thing standing under that path, with QL1.1's bare `{{title}}` as the other route to the same term.

**`/department/sales/` stays among the context pages** (`ctx-term`). It asserts `body_class` `tax-department` rather than the generic `archive`: the term archive is the only context where an ambient read and a stated term source can be told apart at all, so the row must fail if the page degrades into a different kind of archive.

**C-CONV11 is load-bearing for the stated column.** §C-TERM names it as its surface, so if §C-CONV is ever retired the row has to be kept or re-homed rather than deleted with the section.

## §C-CONV — a `term_*` tag beside the base tag it converts to (FW-39)

The C-TERM rows above measure the resolution itself, in the spelling that survived. These measure a tag against its REPLACEMENT: the migration rewrites a `term_*` tag into a base tag, and what has to be shown is not that either side is right but which way every difference between them runs. A single row cannot show a direction, so every row here is half of a pair and is useless read alone.
...
**QL1.6 is the convert-side twin of QL1.5, and it is a fixture row because it cannot be anything else.** A term query loop hands its row's term down through GB's `generateblocks_dynamic_tag_id` filter, and `render-tag` cannot fake a term loop at any flag combination — so the two routes to one term (QL1.5 through `TaxonomyTerm::resolve_id()`, QL1.6 through `bws_resolve_base_source()`'s loop-item classification) were comparable on the page and nowhere else. That is the same gap QL1.5 itself was added to fill, one guard over. **QL1.5's visible row was deleted with the family in 1.21.0** and the measurement above is the record of it; QL1.6 is on the page alone now, against QL1.1's bare `{{title}}` as the second route (§C-TERM/CT).
