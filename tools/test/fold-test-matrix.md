...
| F9b.4e | `{{try_datetime_range A:src(refs,link_staff)\|startKey:event_datetime\|endKey:event_end_datetime\|linkTo:permalink}}` on `/matrix-links/` (row LK.10) | the two ranges joined and UNLINKED → each whole range anchored to the post it was read off | **FW-136 ticket 08**, the fourth and last family FW-135 dissolves on, and with it no `supports_try` template resolves through the arm path at all. What this row adds over F9b.4d: a range is TWO reads already joined by `rangeSep` before the fold sees the value, so it is the one row where the per-item wrap goes around an assembled pair — the anchor wraps the whole range and `sep` still joins whole ranges between anchors. Base twin LK.9, which this row must match byte for byte; two values and not three for F9b.4d's reason |
