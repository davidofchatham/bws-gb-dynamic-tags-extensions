...
| F9b.4d | `{{try_datetime_single A:src(refs,link_staff)\|key:event_datetime\|linkTo:permalink}}` on `/matrix-links/` (row LK.8) | the two dates joined and UNLINKED → each date anchored to the post it was read off | **FW-136 ticket 07**, the third family FW-135 dissolves on. What this row adds over F9b.4b, which is why it is worth a third: F9b.4b's values ARE each entity's own title, so a build taking the link from the VALUE rather than from the source would still render it correctly. A date says nothing about where it lives. Base twin LK.7, which this row must match byte for byte; two values and not three because Fixture Ref Target carries no `event_datetime` and the fold drops an empty read |
...
| F19.4 | `{{try_permalink}}` / `{{try_image}}` / `{{try_datetime_single key:event_date}}` | **empty** — the six families that never carried a `try_query_fn`, and the value is the same empty by two different routes. `permalink` and `image` reach the POST arm (through the base seam once flipped, through the arm shim's fn-absent fallthrough while not), which resolves no id off a query-context base and cannot serve off a loop item on an archive. `datetime_single` stopped taking that route at FW-136 ticket 07: its base seam CLAIMS the query-context kind, and `bws_base_query_context_analog_read()` carries no datetime case, so it answers `''` one layer earlier. Empty on every route, never a leaked entity (pre-1.19.0 these read the main query's first row) |
