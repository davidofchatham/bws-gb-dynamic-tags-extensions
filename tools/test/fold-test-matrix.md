...
| F14.14b | Open a CHAIN-wire tag that still carries a tag-level `limit` — `{{text src:terms,department\|use:title\|limit:1}}`, i.e. hand-written or reached by no migration path — and close it WITHOUT changing anything | the key SURVIVES untouched, and the tag keeps rendering `Sales` where the same chain without it renders `Sales, Support` (both MEASURED), so the assertion is a visible difference rather than a reading of the wire. This is the half F14.14 cannot show: a converting commit is the only thing that rewrites a limit, so on wire that is already a chain there is nothing to migrate and nothing that may clear the key. It is also the shape that proves removing an OPTION does not remove its VALUE — GB re-serializes state it seeded from the tag string, with no registry entry involved |
---
