...
| F9.5f | — | `{{email src:rows,team_members\|key:email\|noLink:1}}` | displays `alice@example.test, bob@example.test` (antispambot entity-encodes the characters; a browser shows the addresses). `limit:1` displays alice alone. **THE BASE TAG NEEDED NO ARM AND THIS ROW IS WHY IT IS WRITTEN DOWN.** `{{email}}` reads through `bws_resolve_field_values()`, which dispatches nothing on source kind and hands every resolved source to the L2 seam, whose `meta_row` case has been live since 1.17.0 — so this rendered the same two addresses on the tree BEFORE ticket 04, measured. Nothing pinned it and no doc said it; that is the gap the row closes, not a behaviour change |
...
| F9.5l | — | `{{image src:rows,team_members\|use:key\|key:feature_image\|as:url}}` | EMPTY. Control: `{{image use:key\|key:feature_image\|as:url}}` renders the fixture photo URL — the page's own `feature_image`, which is exactly what the refusing row printed before 1.21.0. **A KEYED read leaks the same way an analog does**, which is why this pair exists and not just F9.5j's: the leak was never confined to analog spellings, it was the post tail's empty-fan leg, shared by every `use`. `{{image}}` gains a row arm in FW-74 ticket 05, which replaces this refusal with a read |
| F9.5m | — | `{{datetime_single src:rows,team_members\|key:event_date_dmy}}` | EMPTY. Control: `{{datetime_single key:event_date_dmy}}` renders `15/08/2030`, the page's own field and this row's pre-1.21.0 output. Row arm comes with FW-74 ticket 06 |
| F9.5n | — | `{{datetime_range startKey:event_start_date\|endKey:event_end_date\|src:rows,team_members}}` | EMPTY, and it is the row nobody would think to add: `{{datetime_range}}` is a SEPARATE family with its own refusal call site, so a `$serves` list wrong there goes unpinned by F9.5m. Control: the same tag without the chain renders `August 1–9, 2030` |
