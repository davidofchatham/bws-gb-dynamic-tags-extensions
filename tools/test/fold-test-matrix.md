...
|  |  | **`bio` IS ON NO FIXTURE, AND THAT IS FORCED — read this before "improving" the keys.** A base tag with an unresolvable source does NOT render nothing: it falls through to the AMBIENT entity and reads it. Measured 2026-08-15 on `/matrix-post-meta/` — `{{text src:currnet\|use:key\|key:name_first}}` renders `Jane`, and so do the retired-token and unknown-step spellings, while `{{phone src:refs,related_staff;bogus,x\|key:main_line}}` correctly renders empty (§F11.1). An [I15] leak on the text path, filed as [#109](https://github.com/davidofchatham/bws-gb-dynamic-tags-extensions/issues/109). Two consequences here: the preview is built ONLY where resolution came back empty, so a row keyed on a field the page HAS would render a plausible wrong value and show no preview at all; and every row above therefore names a field nothing carries. When the leak is fixed the keys can go back and the rows keep working. **Run status:** the five tags were swept with `render-tag` and all five resolve empty; the blueprint was reseeded and all five blocks curl back on `/matrix-post-meta/` (2026-08-15). The EDITOR read — the bracket text itself, which is the whole point of these rows — is outstanding |
---
