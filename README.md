...
**[UNRELEASED]** A repeater field is a source too. Point a `text`, `content`, `email` or `phone` tag at one and it renders a named sub-field from its rows, and `join` fields and `try_` attempts read a repeater the same way. `text`, `email` and `phone` render every row, joined and bounded by the same Result Separator and limit those tags use everywhere else; `content` renders the first row, which is what it does with a list of posts too. A row is not a post or a term, so it has no title, address or date of its own: name the sub-field you want. Repeater steps are not in the source builder yet, so this is hand-typed wire for now (`{{text src:rows,team_members|key:name}}`), and `image` and the two `datetime` tags still return nothing from a row.
