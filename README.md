...
**[UNRELEASED]** A repeater field is a source too. Point a `text`, `content`, `image`, `email` or `phone` tag at one and it renders a named sub-field from its rows, and `join` fields and `try_` attempts read a repeater the same way. `text`, `email` and `phone` render every row, joined and bounded by the same Result Separator and limit those tags use everywhere else; `content` and `image` render the first row, which is what they do with a list of posts too. An `image` sub-field works whichever return format its field uses, and the Return As and size choices behave exactly as they do on a post. A row is not a post or a term, so it has no title, address, featured image or date of its own: name the sub-field you want. Repeater steps are not in the source builder yet, so this is hand-typed wire for now (`{{text src:rows,team_members|key:name}}`), and the two `datetime` tags still return nothing from a row.
